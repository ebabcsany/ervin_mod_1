      ...
      if (!(blockstateLocation instanceof ModelResourceLocation)) {
         ...
         if (Objects.equals(modelresourcelocation.getVariant(), "inventory")) {
            ...
            try {
               ...
               try {
                  list1 = this.resourceManager.getAllResources(resourcelocation1).stream().map((p_229345_1_) -> {
                     try (InputStream inputstream = p_229345_1_.getInputStream()) {
                        ...
                        throw new BlockStateDefinitionException(String.format("Exception loading blockstate definition: '%s' in resourcepack: '%s': %s", p_229345_1_.getLocation(), p_229345_1_.getPackName(), exception1.getMessage()));
                     }
                  }).collect(Collectors.toList());
               } catch (IOException ioexception) {
                  ...
                  if (blockmodeldefinition.hasMultipartData()) {
                     ...
                     immutablelist.forEach((p_229339_3_) -> {
                        Pair pair2 = map2.put(p_229339_3_, Pair.of(multipart, () -> {
                           return ModelListWrapper.makeWrapper(p_229339_3_, multipart, list);
                        }));
                     });
                  } else {
                     multipart = null;
                  ...
                  blockmodeldefinition.getVariants().forEach((p_229329_9_, p_229329_10_) -> {
                     try {
                        immutablelist.stream().filter(parseVariantKey(statecontainer, p_229329_9_)).forEach((p_229338_6_) -> {
                           Pair<IUnbakedModel, Supplier<ModelListWrapper>> pair2 = map2.put(p_229338_6_, Pair.of(p_229329_10_, () -> {
                              return ModelListWrapper.makeWrapper(p_229338_6_, p_229329_10_, list);
                           }));
               ...
               HashMap<ModelListWrapper, Set<BlockState>> lvt_20_1_ = Maps.newHashMap();
               map.forEach((p_229341_5_, p_229341_6_) -> {
                  Pair<IUnbakedModel, Supplier<ModelListWrapper>> pair2 = map1.get(p_229341_6_);
                  if (pair2 == null) {
