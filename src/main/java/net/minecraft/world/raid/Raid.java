...
/*public class Raid {
   ...
   private final Map<Integer, AbstractZurEntity> leaderss = Maps.newHashMap();
   private final Map<Integer, Set<AbstractRaiderEntity>> raiders = Maps.newHashMap();
   private final Map<Integer, Set<AbstractZurEntity>> raiderss = Maps.newHashMap();
   ...
   private void spawnNextWave1(BlockPos p_221294_1_) {
      boolean flag = false;
      int i = this.groupsSpawned + 1;
      this.totalHealth = 0.0F;
      DifficultyInstance difficultyinstance = this.world.getDifficultyForLocation(p_221294_1_);
      ...
      for(WaveMemberr raid$wavemember : WaveMemberr.VALUES) {
         int j = this.getDefaultNumSpawnss(raid$wavemember, i, flag1) + this.getPotentialBonusSpawnss(raid$wavemember, this.random, i, difficultyinstance, flag1);
         int k = 0;

         for(int l = 0; l < j; ++l) {
            AbstractZurEntity abstractraiderentity = raid$wavemember.type.create(this.world);
            if (!flag && abstractraiderentity.canBeLeader()) {
               abstractraiderentity.setLeader(true);
               this.setLeaderZur(i, abstractraiderentity);
               flag = true;
            }
            ...
            if (raid$wavemember.type == EntityInit.DGRURB_ENTITY.get()) {
               ...
               ++k;
               if (abstractraiderentity1 != null) {
                  this.joinRaidd(i, abstractraiderentity1, p_221294_1_, false);
                  abstractraiderentity1.moveToBlockPosAndAngles(p_221294_1_, 0.0F, 0.0F);
                  abstractraiderentity1.startRiding(abstractraiderentity);
               }
            }
   ...
   public void joinRaidd(int wave, AbstractZurEntity p_221317_2_, @Nullable BlockPos p_221317_3_, boolean p_221317_4_) {
      boolean flag = this.joinRaidd(wave, p_221317_2_);
      if (flag) {
         p_221317_2_.setRaid(this);
         p_221317_2_.setWave(wave);
         p_221317_2_.func_213644_t(true);
   ...
   private boolean joinRaidd(int p_221287_1_, AbstractZurEntity p_221287_2_) {
      return this.joinRaidd(p_221287_1_, p_221287_2_, true);
   }

   public boolean joinRaid(int p_221300_1_, AbstractRaiderEntity p_221300_2_, boolean p_221300_3_) {
      this.raiderss.computeIfAbsent(p_221300_1_, (p_221323_0_) -> {
   ...
   public boolean joinRaidd(int p_221300_1_, AbstractZurEntity zur, boolean p_221300_3_) {
      this.raiderss.computeIfAbsent(p_221300_1_, (p_221323_0_) -> {
         return Sets.newHashSet();
      });
      Set<AbstractZurEntity> set = this.raiderss.get(p_221300_1_);
      AbstractZurEntity abstractraiderentity = null;

      for(AbstractZurEntity abstractraiderentity1 : set) {
         if (abstractraiderentity1.getUniqueID().equals(zur.getUniqueID())) {
            abstractraiderentity = abstractraiderentity1;
            break;
         }
      }
      ...
      set.add(zur);
      if (p_221300_3_) {
         this.totalHealth += zur.getHealth();
      }
   ...
   public void setLeaderZur(int raidId, AbstractZurEntity p_221324_2_) {
      this.leaderss.put(raidId, p_221324_2_);
      p_221324_2_.setItemStackToSlot(EquipmentSlotType.HEAD, createIllagerBanner());
      p_221324_2_.setDropChance(EquipmentSlotType.HEAD, 2.0F);
   }
   ...
   private int getDefaultNumSpawnss(WaveMemberr p_221330_1_, int p_221330_2_, boolean p_221330_3_) {
      return p_221330_3_ ? p_221330_1_.waveCounts[this.numGroups] : p_221330_1_.waveCounts[p_221330_2_];
   }

   private int getPotentialBonusSpawnss(WaveMemberr p_221335_1_, Random p_221335_2_, int wave, DifficultyInstance p_221335_4_, boolean p_221335_5_) {
      Difficulty difficulty = p_221335_4_.getDifficulty();
      boolean flag = difficulty == Difficulty.EASY;
      boolean flag1 = difficulty == Difficulty.NORMAL;
      int i;
   ...
   public static enum WaveMemberr implements net.minecraftforge.common.IExtensibleEnum {
      ...
      private WaveMemberr(EntityType<? extends AbstractZurEntity> typeIn, int[] waveCountsIn) {
         this.type = typeIn;
         this.waveCounts = waveCountsIn;
      }
