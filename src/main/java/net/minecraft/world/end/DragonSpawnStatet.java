...
public enum DragonSpawnStatet {
   START {
      public void process(ServerWorld worldIn, DragonFightManagerl manager, List<EnderCrystalEntity> crystals, int ticks, BlockPos pos) {
      ...
      public void process(ServerWorld worldIn, ModDragonFightManager manager, List<EnderCrystalEntity> crystals, int ticks, BlockPos pos) {
         BlockPos blockpos = new BlockPos(0, 128, 0);
         ...
         if (flag || flag1) {
            ...
            if (j < list.size()) {
               ...
               if (flag) {
                  ...
                  worldIn.createExplosion(null, (float)endspikefeature$endspike.getCenterX() + 0.5F, (double)endspikefeature$endspike.getHeight(), (double)((float)endspikefeature$endspike.getCenterZ() + 0.5F), 5.0F, Explosion.Mode.DESTROY);
                  EndSpikeFeatureConfig endspikefeatureconfig = new EndSpikeFeatureConfig(true, ImmutableList.of(endspikefeature$endspike), new BlockPos(0, 128, 0));
                  Feature.END_SPIKE.withConfiguration(endspikefeatureconfig).func_236265_a_(worldIn, worldIn.func_241112_a_(), worldIn.getChunkProvider().getChunkGenerator(), new Random(), new BlockPos(endspikefeature$endspike.getCenterX(), 45, endspikefeature$endspike.getCenterZ()));
               }
            } else if (flag) {
   ...
   private DragonSpawnStatet() {
   }

   public abstract void process(ServerWorld worldIn, DragonFightManagerl manager, List<EnderCrystalEntity> crystals, int ticks, BlockPos pos);

   public abstract void process(ServerWorld worldIn, ModDragonFightManager manager, List<EnderCrystalEntity> crystals, int ticks, BlockPos pos);
}
