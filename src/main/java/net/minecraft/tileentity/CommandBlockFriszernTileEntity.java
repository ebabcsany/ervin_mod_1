...
public class CommandBlockFriszernTileEntity extends TileEntity {
   ...
   private final CommandBlockLogic commandBlockLogic = new CommandBlockLogic() {
      ...
      public void setCommand(String command) {
         ...
         CommandBlockFriszernTileEntity.this.markDirty();
      }

      public ServerWorld getWorld() {
         return (ServerWorld) CommandBlockFriszernTileEntity.this.world;
      }

      public void updateCommand() {
         BlockState blockstate = CommandBlockFriszernTileEntity.this.world.getBlockState(CommandBlockFriszernTileEntity.this.pos);
         this.getWorld().notifyBlockUpdate(CommandBlockFriszernTileEntity.this.pos, blockstate, blockstate, 3);
      }

      @OnlyIn(Dist.CLIENT)
      public Vector3d getPositionVector() {
         return Vector3d.copyCentered(CommandBlockFriszernTileEntity.this.pos);
      }

      public CommandSource getCommandSource() {
         return new CommandSource(this, Vector3d.copyCentered(CommandBlockFriszernTileEntity.this.pos), Vector2f.ZERO, this.getWorld(), 2, this.getName().getString(), this.getName(), this.getWorld().getServer(), (Entity)null);
      }
   };

   public CommandBlockFriszernTileEntity() {
    ...
    * CommandBlockFriszernTileEntity#onlyOpsCanSetNbt() command blocks} are considered
   ...
   public void setAuto(boolean autoIn) {
      ...
      if (!flag && autoIn && !this.powered && this.world != null && this.getFriszernMode() != Mode.SEQUENCE) {
         this.func_226988_y_();
      }
   ...
   public void func_226987_h_() {
      ...
      Mode commandblocktileentity$friszernmode = this.getFriszernMode();
      if (commandblocktileentity$mode == Mode.AUTO && (this.powered || this.auto) && this.world != null) {
         this.func_226988_y_();
      }
      if (commandblocktileentity$friszernmode == Mode.AUTO && (this.powered || this.auto) && this.world != null) {
         this.func_226988_y_();
      }
   ...
   public boolean setConditionMet() {
      ...
      if (this.isConditional()) {
         ...
         if (this.world.getBlockState(blockpos).getBlock() instanceof CommandBlockBlock) {
            ...
            this.conditionMet = tileentity instanceof CommandBlockFriszernTileEntity && ((CommandBlockFriszernTileEntity)tileentity).getCommandBlockLogic().getSuccessCount() > 0;
         } else {
   ...
   public Mode getFriszernMode() {
      ...
      if (blockstate.isIn(BlockInit.FRISZERN)) {
         ...
         return blockstate.isIn(BlockInit.CHAIN_FRISZERN) ? Mode.SEQUENCE : Mode.REDSTONE;
      }
   }
