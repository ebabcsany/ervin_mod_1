...
public class ModBlock extends Block implements IItemProvider, net.minecraftforge.common.extensions.IForgeBlock {
   ...
   @Nullable
   private Item item;
   private static final ThreadLocal<Object2ByteLinkedOpenHashMap<ModBlock.RenderSideCacheKey>> SHOULD_SIDE_RENDER_CACHE = ThreadLocal.withInitial(() -> {
      Object2ByteLinkedOpenHashMap<ModBlock.RenderSideCacheKey> object2bytelinkedopenhashmap = new Object2ByteLinkedOpenHashMap<ModBlock.RenderSideCacheKey>(2048, 0.25F) {
   ...
   public ModBlock(String name, Material material, int harvestLevel, ToolType harvestTool, float hardnessAndResistance) {
      super(AbstractBlock.Properties.create(material));
      Ervin_mod_1.BLOCKS.register(name, () -> new ModBlock(AbstractBlock.Properties.create(material).setRequiresTool().harvestLevel(harvestLevel).harvestTool(harvestTool).hardnessAndResistance(hardnessAndResistance)));
   }
   ...
   public static void func_241468_a_(BlockState oldState, BlockState newState, IWorld world, BlockPos pos, int p_241468_4_, int p_241468_5_) {
      if (newState != oldState) {
         if (newState.isAir()) {
            if (!world.isRemote()) {
               world.destroyBlock(pos, (p_241468_4_ & 32) == 0, null, p_241468_5_);
            }
         } else {
   ...
   public static boolean shouldSideBeRendered(BlockState adjacentState, IBlockReader blockState, BlockPos blockAccess, Direction pos) {
      ...
      if (adjacentState.isSideInvisible(blockstate, pos)) {
         ...
         ModBlock.RenderSideCacheKey block$rendersidecachekey = new ModBlock.RenderSideCacheKey(adjacentState, blockstate, pos);
         Object2ByteLinkedOpenHashMap<ModBlock.RenderSideCacheKey> object2bytelinkedopenhashmap = SHOULD_SIDE_RENDER_CACHE.get();
   ...
   public static List<ItemStack> getDrops(BlockState state, ModServerWorld worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn) {
      LootContext.Builder lootcontext$builder = (new LootContext.Builder(worldIn.getWorldServer())).withRandom(worldIn.rand).withParameter(LootParameters.POSITION, pos).withParameter(LootParameters.TOOL, ItemStack.EMPTY).withNullableParameter(LootParameters.BLOCK_ENTITY, tileEntityIn);
      return state.getDrops(lootcontext$builder);
   }

   public static List<ItemStack> getDrops(BlockState state, ModServerWorld worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn, @Nullable Entity entityIn, ItemStack stack) {
      LootContext.Builder lootcontext$builder = (new LootContext.Builder(worldIn.getWorldServer())).withRandom(worldIn.rand).withParameter(LootParameters.POSITION, pos).withParameter(LootParameters.TOOL, stack).withNullableParameter(LootParameters.THIS_ENTITY, entityIn).withNullableParameter(LootParameters.BLOCK_ENTITY, tileEntityIn);
      return state.getDrops(lootcontext$builder);
   }

   public static void spawnDrops(BlockState state, world worldIn, BlockPos pos) {
      if (worldIn instanceof ModServerWorld) {
         getDrops(state, (ModServerWorld)worldIn, pos, null).forEach((p_220079_2_) -> spawnAsEntity(worldIn.getWorld(), pos, p_220079_2_));
      }

      state.spawnAdditionalDrops(worldIn.getWorld(), pos, ItemStack.EMPTY);
   }

   public static void spawnDrops(BlockState state, world worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn) {
      if (worldIn instanceof ModServerWorld) {
         getDrops(state, (ModServerWorld)worldIn, pos, tileEntityIn).forEach((p_220061_2_) -> spawnAsEntity(worldIn.getWorld(), pos, p_220061_2_));
      }
   ...
   public static void spawnDrops(BlockState state, world worldIn, BlockPos pos, @Nullable TileEntity tileEntityIn, Entity entityIn, ItemStack stack) {
      if (worldIn instanceof ModServerWorld) {
         getDrops(state, (ModServerWorld)worldIn, pos, tileEntityIn, entityIn, stack).forEach((p_220057_2_) -> spawnAsEntity(worldIn.getWorld(), pos, p_220057_2_));
      }

      state.spawnAdditionalDrops(worldIn.getWorld(), pos, stack);
   }
   ...
   public static void spawnAsEntity(world worldIn, BlockPos pos, ItemStack stack) {
      if (!worldIn.isRemote && !stack.isEmpty() && worldIn.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && !worldIn.restoringBlockSnapshots) {
         ...
         ItemEntity itementity = new ItemEntity(worldIn.getWorld(), (double)pos.getX() + d0, (double)pos.getY() + d1, (double)pos.getZ() + d2, stack);
   ...
   public void dropXpOnBlockBreak(world worldIn, BlockPos pos, int amount) {
      if (!worldIn.isRemote && worldIn.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && !worldIn.restoringBlockSnapshots) {
         while(amount > 0) {
            ...
            amount -= i;
            worldIn.addEntity(new ExperienceOrbEntity(worldIn.getWorld(), (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D, i));
         }
      }
    ...
    * Spawns the given amount of experience into the world as XP orb entities
    */
   public void dropEntityOnBlockBreak(Entity entity, world worldIn, BlockPos pos, int amount) {
      if (!worldIn.isRemote && worldIn.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && !worldIn.restoringBlockSnapshots) {
   ...
   public boolean canSustainPlant(BlockState state, IBlockReader world, BlockPos pos, Direction facing, net.minecraftforge.common.IPlantable plantable) {
      ...
      if (net.minecraftforge.common.PlantType.DESERT.equals(type)) {
         ...
         return ModBlock.hasSolidSide(state, world, pos, Direction.UP);
