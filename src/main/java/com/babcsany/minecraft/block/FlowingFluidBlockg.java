...
public class FlowingFluidBlockg extends Block implements IBucketPickupHandler {
   ...
   public FlowingFluidBlockg(FlowingFluid fluidIn, Properties builder) {
      super(builder);
      this.fluid = fluidIn;
    ...
    */
   public FlowingFluidBlockg(java.util.function.Supplier<? extends FlowingFluid> supplier, Properties properties) {
      super(properties);
      this.fluid = null;
      this.field_212565_c = Lists.newArrayList();
      this.setDefaultState(this.stateContainer.getBaseState().with(LEVEL, Integer.valueOf(0)));
         ...
         for(Direction direction : Direction.values()) {
            if (direction != Direction.DOWN) {
               ...
               if (flag && worldIn.getBlockState(blockpos).isIn(Blocks.BLUE_ICE)) {
                  worldIn.setBlockState(pos, net.minecraftforge.event.ForgeEventFactory.fireFluidPlaceBlockEvent(worldIn, pos, pos, BlockInit.GRINT_BLOCK.get().getDefaultState()));
                  this.triggerMixEffects(worldIn, pos);
                  return false;
               }
            }
