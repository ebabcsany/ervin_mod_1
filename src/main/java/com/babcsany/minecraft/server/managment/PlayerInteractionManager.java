package com.babcsany.minecraft.server.managment;
...
public class PlayerInteractionManager {
   ...
   public boolean tryHarvestBlock(BlockPos pos) {
      ...
      if (exp == -1) {
         ...
         if ((block instanceof CommandBlockBlock || block instanceof StructureBlock || block instanceof JigsawBlock) && !this.player.canUseCommandBlock()) {
            ...
            return false;
         } else if (living.getHeldIcsvreMainhand().onBlockStartBreak(pos, player)) {
            return false;
            ...
            if (this.isCreative()) {
               ...
               IcsvreStack itemstack = this.living.getHeldIcsvreMainhand();
               IcsvreStack itemstack1 = itemstack.copy();
               ...
               if (itemstack.isEmpty() && !itemstack1.isEmpty())
                  ForgeEventFactory.onPlayerDestroyIcsvre(this.player, itemstack1, Hand.MAIN_HAND);
               boolean flag = removeBlock(pos, flag1);

               if (flag && flag1) {
                  block.harvestBlock(this.world, this.player, pos, blockstate, tileentity, itemstack1.getStack());
               }
   ...
   public ActionResultType processRightClick(ServerPlayerEntity player, World worldIn, IcsvreStack stack, Hand hand) {
      if (this.gameType == GameType.SPECTATOR) {
         return ActionResultType.PASS;
      } else if (player.getCooldownTracker().hasCooldown(stack.getIcsvre().item)) {
         return ActionResultType.PASS;
      } else {
         ...
         ActionResult<IcsvreStack> actionresult = stack.useItemRightClick(worldIn, player, hand);
         IcsvreStack itemstack = actionresult.getResult();
         if (itemstack == stack && itemstack.getCount() == i && itemstack.getUseDuration() <= 0 && itemstack.getDamage() == j) {
            ...
            player.setHeldItem(hand, itemstack.getStack());
            if (this.isCreative()) {
            ...
            if (itemstack.isEmpty()) {
               living.setHeldIcsvre(hand, IcsvreStack.EMPTY);
            }
   ...
   public ActionResultType func_219441_a(ServerPlayerEntity playerIn, World worldIn, IcsvreStack stackIn, Hand handIn, BlockRayTraceResult blockRaytraceResultIn) {
      ...
      if (this.gameType == GameType.SPECTATOR) {
         ...
         IcsvreUseContext itemusecontext = new IcsvreUseContext(playerIn, handIn, blockRaytraceResultIn);
         ...
         IcsvreStack icsvreStack = stackIn.copy();
         if (event.getUseBlock() != net.minecraftforge.eventbus.api.Event.Result.DENY && !flag1) {
            ...
            if (actionresulttype.isSuccessOrConsume()) {
               CriteriaTriggers1.RIGHT_CLICK_BLOCK_WITH_ICSVRE.test(playerIn, blockpos, icsvreStack);
               return actionresulttype;
            }
         }

         if (!stackIn.isEmpty() && !playerIn.getCooldownTracker().hasCooldown(stackIn.getIcsvre().item)) {
            ...
            if (actionresulttype1.isSuccessOrConsume()) {
               CriteriaTriggers.RIGHT_CLICK_BLOCK_WITH_ITEM.test(playerIn, blockpos, icsvreStack.getStack());
            }
