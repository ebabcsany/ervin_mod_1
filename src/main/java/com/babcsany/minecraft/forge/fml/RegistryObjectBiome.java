...
package com.babcsany.minecraft.forge.fml;
...
public final class RegistryObjectBiome<T extends IForgeRegistryEntry<? super T>> implements Supplier<T>
{
    private final ResourceLocation name;
    @Nullable
    private T value;

    public static <T extends IForgeRegistryEntry<T>, U extends T> RegistryObjectBiome<U> of(final ResourceLocation name, Supplier<Class<? super T>> registryType) {
        return new RegistryObjectBiome<>(name, registryType);
    }

    public static <T extends IForgeRegistryEntry<T>, U extends T> RegistryObjectBiome<U> of(final ResourceLocation name, IForgeRegistry<T> registry) {
        return new RegistryObjectBiome<>(name, registry);
    }

    public static <T extends IForgeRegistryEntry<T>, U extends T> RegistryObjectBiome<U> of(final ResourceLocation name, final Class<T> baseType, String modid) {
        return new RegistryObjectBiome<>(name, baseType, modid);
    }

    private static RegistryObjectBiome<?> EMPTY = new RegistryObjectBiome<>();

    private static <T extends IForgeRegistryEntry<? super T>> RegistryObjectBiome<T> empty() {
        @SuppressWarnings("unchecked")
        RegistryObjectBiome<T> t = (RegistryObjectBiome<T>) EMPTY;
        return t;
    }

    private RegistryObjectBiome() {
        this.name = null;
    }

    private <V extends IForgeRegistryEntry<V>> RegistryObjectBiome(ResourceLocation name, Supplier<Class<? super V>> registryType)
    {
        this(name, RegistryManager.ACTIVE.<V>getRegistry(registryType.get()));
    }

    @SuppressWarnings("unchecked")
    private <V extends IForgeRegistryEntry<V>> RegistryObjectBiome(ResourceLocation name, IForgeRegistry<V> registry)
    {
        if (registry == null)
    ...
    @SuppressWarnings("unchecked")
    private <V extends IForgeRegistryEntry<V>> RegistryObjectBiome(final ResourceLocation name, final Class<V> baseType, final String modid)
    {
        this.name = name;
        ...
        ObjectHolderRegistry.addHandler(new Consumer<Predicate<ResourceLocation>>()
        {
            ...
            public void accept(Predicate<ResourceLocation> pred)
            {
                ...
                if (pred.test(registry.getRegistryName()))
                    RegistryObjectBiome.this.value = registry.containsKey(RegistryObjectBiome.this.name) ? (T)registry.getValue(RegistryObjectBiome.this.name) : null;
            }
        });
    }
     ...
     */
    public RegistryObjectBiome<T> filter(Predicate<? super T> predicate) {
    ...
    public boolean equals(Object obj)
    {
        ...
        if (obj instanceof RegistryObjectBiome) {
            return Objects.equals(((RegistryObjectBiome<?>)obj).name, name);
        }
        return false;
    }
