...
public class EntityInit<T extends Entity> extends EntityType<T> {
    ...
    public static final EntityType<DrurbFishEntity> DRURB_ENTITY = register("drurb", EntityType.Builder.create(DrurbFishEntity::new, EntityClassification.CREATURE).size(0.6f, 0.5f).trackingRange(8).immuneToFire());
    public static final EntityType<ZurEntity> ZUR_ENTITY = register("zur", EntityType.Builder.create(ZurEntity::new, EntityClassification.CREATURE).size(0.6f, 1.95f).trackingRange(10).immuneToFire());
    ...
    public static <T extends Entity> EntityType<T> register(String key, EntityType.Builder<T> builder) {
        ...
        return Registry.register(Registry.ENTITY_TYPE, identifier, builder.build(identifier));
    }
}
