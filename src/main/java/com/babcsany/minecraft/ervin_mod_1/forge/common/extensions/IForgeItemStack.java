...
public interface IForgeItemStack extends ICapabilitySerializable<CompoundNBT>
{
    ...
    default IcsvreStack getStack()
    {
        return (IcsvreStack.EMPTY.copy());
    }

    IcsvreUseContext context_ = IcsvreUseContext.icsvreUseContext;

    /**
     * ItemStack sensitive version of getContainerItem. Returns a full ItemStack
     * instance of the result.
     *
     * @param itemStack The current IcsvreStack
     * @return The resulting ItemStack
     */
    default IcsvreStack getContainerItem(IcsvreStack itemStack)
    {
        return getStack().getIcsvre().getContainerItem(getStack());
    }
    ...
    {
        return getStack().getIcsvre().hasContainerItem(getStack());
    }
    ...
    {
        return getStack().getIcsvre().getBurnTime(getContainerItem(this.getStack()).getStack());
    }
    ...
    {
        return getStack().getIcsvre().getHarvestLevel(getStack(), tool, player, state);
    }
    ...
    default ActionResultType onItemUseFirst(ModItemUseContext context)
    {
       ...
       if (entityplayer != null && !entityplayer.abilities.allowEdit && !getStack().canPlaceOn(context.getWorld().getTags(), blockworldstate)) {
          ...
          Icsvre item = getStack().getIcsvre();
          ActionResultType enumactionresult = item.onIcsvreUseFirst(getStack(), context_);
          if (entityplayer != null && enumactionresult == ActionResultType.SUCCESS) {
             entityplayer.addStat(Stats.ITEM_USED.get(this.getStack().getItem()));
          }
    ...
    {
        return !getStack().isEmpty() && getStack().getIcsvre().onBlockStartBreak(getStack().getStack(), pos, player);
    }
    ...
    {
        return getStack().getIcsvre().shouldCauseBlockBreakReset(getStack().getStack(), newStack);
    }
    ...
    {
        return getStack().getIcsvre().canApplyAtEnchantingTable(getStack().getStack(), enchantment);
    }
    ...
    {
        return getStack().getIcsvre().getItemEnchantability(getStack().getStack());
    }
    ...
    {
        return getStack().getIcsvre().getEquipmentSlot(getStack().getStack());
    }
    ...
    {
        return getStack().getIcsvre().canDisableShield(getStack().getStack(), shield, entity, attacker);
    }
    ...
    {
        return getStack().getIcsvre().isShield(getStack().getStack(), entity);
    }
    ...
    {
        return getStack().getIcsvre().onEntitySwing(getStack().getStack(), entity);
    }
    ...
    {
        getStack().getIcsvre().onUsingTick(getStack().getStack(), player, count);
    }
    ...
    {
        return getStack().getIcsvre().getEntityLifespan(getStack(), world);
    }
     ...
     */
    default boolean onEntityItemUpdate(IcsvreEntity entity)
    {
        return getStack().getIcsvre().onEntityItemUpdate(getStack().getStack(), entity);
    }
    ...
    */
    ...
    {
        return getStack().getIcsvre().getXpRepairRatio(getStack().getStack());
    }
    ...
    {
        getStack().getIcsvre().onArmorTick(getStack().getStack(), world, player);
    }

    /**
     * Called every tick from {@link HorseEntity#onDeathUpdate()} on the item in the
     * armor slot.
     *
    ...
    {
        getStack().getIcsvre().onHorseArmorTick(getStack().getStack(), world, horse);
    }
    ...
    {
        return getStack().getIcsvre().canEquip(getStack().getStack(), armorType, entity);
    }
    ...
    {
        return getStack().getIcsvre().isBookEnchantable(getStack().getStack(), book);
    }
    ...
    {
        return getStack().getIcsvre().onDroppedByPlayer(getStack(), player);
    }
    ...
    {
        return getStack().getIcsvre().getHighlightTip(getStack(), displayName);
    }
    ...
    {
        return getStack().getIcsvre().getShareTag(getStack().getStack());
    }
    ...
    {
        getStack().getIcsvre().readShareTag(getStack().getStack(), nbt);
    }
    ...
    {
        return getStack().isEmpty() || getStack().getIcsvre().doesSneakBypassUse(getStack().getStack(), world, pos, player);
    }
    ...
    default boolean equals(ItemStack other, boolean limitTags)
    {
        if (getStack().isEmpty())
            ...
            return !other.isEmpty() && getStack().getCount() == other.getCount() && getStack().getIcsvre().getItem() == other.getItem() &&
            (limitTags ? getStack().areShareTagsEqual(other) : ItemStack.areItemStackTagsEqual(getStack().getStack(), other));
    }
    ...
    {
        return getStack().getIcsvre().isRepairable(getStack());
    }
    ...
    {
        return getStack().getIcsvre().isZurCurrency(getStack());
    }
    ...
    {
        return getStack().getIcsvre().makesPiglinsNeutral(getStack().getStack(), wearer);
    }
    ...
    {
        return getStack().getIcsvre().isEnderMask(getStack().getStack(), player, endermanEntity);
    }
    ...
    {
        return getStack().getIcsvre().canElytraFly(getStack().getStack(), entity);
    }
    ...
    {
        return getStack().getIcsvre().elytraFlightTick(getStack().getStack(), entity, flightTicks);
    }
}
