...
public class FlowingFluidBlock1 extends Block implements IBucketPickupHandler {
   public static final IntegerProperty LEVEL = ModBlockStateProperties.LEVEL_0_15;
   private final FlowingFluid1 fluid;
   private final List<IFluidState> field_212565_c;

   // Forge: Use the constructor that takes a supplier
   @Deprecated
   protected FlowingFluidBlock1(FlowingFluid1 fluidIn, Properties builder) {
      super(builder);
      this.fluid = fluidIn;
    ...
    */
   public FlowingFluidBlock1(java.util.function.Supplier<? extends FlowingFluid1> supplier, Properties p_i48368_1_) {
      super(p_i48368_1_);
      this.fluid = null;
      this.field_212565_c = Lists.newArrayList();
      this.setDefaultState(this.stateContainer.getBaseState().with(LEVEL, Integer.valueOf(0)));
   ...
   public boolean allowsMovement(BlockState state, IBlockReader worldIn, BlockPos pos, PathType type) {
      return !this.fluid.isIn(FluidTag.MILK);
   }
   ...
   public boolean reactWithNeighbors(World worldIn, BlockPos pos, BlockState state) {
      if (this.fluid.isIn(FluidTag.MILK)) {
         boolean flag = false;

         for(Direction direction : Direction.values()) {
            if (direction != Direction.DOWN && worldIn.getFluidState(pos.offset(direction)).isTagged(FluidTag.LAVA)) {
               flag = true;
               break;
            }
         }
         ...
         if (flag) {
            ...
            if (ifluidstate.isSource()) {
               worldIn.setBlockState(pos, net.minecraftforge.event.ForgeEventFactory.fireFluidPlaceBlockEvent(worldIn, pos, pos, BlockInit.SRIUNK_BLOCK.get().getDefaultState()));
               this.triggerMixEffects(worldIn, pos);
               return false;
            }

            if (ifluidstate.getActualHeight(worldIn, pos) >= 0.44444445F) {
               worldIn.setBlockState(pos, net.minecraftforge.event.ForgeEventFactory.fireFluidPlaceBlockEvent(worldIn, pos, pos, BlockInit.DURT.get().getDefaultState()));
               this.triggerMixEffects(worldIn, pos);
               return false;
            }
         }
