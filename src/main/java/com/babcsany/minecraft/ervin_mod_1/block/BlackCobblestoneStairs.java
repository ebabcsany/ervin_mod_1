...
public class BlackCobblestoneStairs extends Block implements IWaterLoggable {
   ...
   @Deprecated
   protected BlackCobblestoneStairs(BlockState state, Properties properties) {
      super(properties);
   ...
   public BlackCobblestoneStairs(java.util.function.Supplier<BlockState> state, Properties properties) {
      super(properties);
      this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(HALF, Half.BOTTOM).with(SHAPE, StairsShape.STRAIGHT).with(WATERLOGGED, Boolean.valueOf(false)));
