...
public class JurkFlowingFluid extends Block implements IBucketPickupHandler {
   ...
   @Deprecated
   protected JurkFlowingFluid(FlowingFluid fluidIn, Properties builder) {
      super(builder);
      this.fluid = fluidIn;
    ...
    */
   public JurkFlowingFluid(java.util.function.Supplier<? extends FlowingFluid> supplier, Properties p_i48368_1_) {
      super(p_i48368_1_);
      this.fluid = null;
      this.field_212565_c = Lists.newArrayList();
      this.setDefaultState(this.stateContainer.getBaseState().with(LEVEL, Integer.valueOf(0)));
      ...
      if (this.fluid.isIn(FluidTag.JURK)) {
         ...
         if (flag) {
            ...
            if (ifluidstate.getActualHeight(worldIn, pos) >= 0.44444445F) {
               worldIn.setBlockState(pos, net.minecraftforge.event.ForgeEventFactory.fireFluidPlaceBlockEvent(worldIn, pos, pos, BlockItemInit.DURT.get().getDefaultState()));
               this.triggerMixEffects(worldIn, pos);
               return false;
            }
         }
