...
public class BubbleColumn extends Block implements IBucketPickupHandler {
   public static final BooleanProperty DRAG = BlockStateProperties.DRAG;

   public BubbleColumn(Properties properties) {
      super(properties);
   ...
   public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
      ...
      if (blockstate.isAir()) {
         ...
         if (!worldIn.isRemote) {
            ...
            for(int i = 0; i < 2; ++i) {
               serverworld.spawnParticle(ParticleTypes.SPLASH, (double)pos.getX() + worldIn.rand.nextDouble(), (double)(pos.getY() + 1), (double)pos.getZ() + worldIn.rand.nextDouble(), 1, 0.0D, 0.0D, 0.0D, 10.0D);
               serverworld.spawnParticle(ParticleTypes.BUBBLE, (double)pos.getX() + worldIn.rand.nextDouble(), (double)(pos.getY() + 1), (double)pos.getZ() + worldIn.rand.nextDouble(), 1, 0.0D, 0.01D, 0.0D, 2.0D);
            }
         }
      } else {
   ...
   public static void placeBubbleColumn(IWorld world, BlockPos pos, boolean drag) {
      if (canHoldBubbleColumn(world, pos)) {
         world.setBlockState(pos, Blocks.BUBBLE_COLUMN.getDefaultState().with(DRAG, Boolean.valueOf(drag)), 20);
      }
