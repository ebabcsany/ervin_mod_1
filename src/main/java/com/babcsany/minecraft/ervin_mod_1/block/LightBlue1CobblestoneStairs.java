...
public class LightBlue1CobblestoneStairs extends Block implements IWaterLoggable {
   ...
   public LightBlue1CobblestoneStairs(BlockState state, Properties properties) {
      super(properties);
   ...
   public LightBlue1CobblestoneStairs(java.util.function.Supplier<BlockState> state, Properties properties) {
      super(properties);
      this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(HALF, Half.BOTTOM).with(SHAPE, StairsShape.STRAIGHT).with(WATERLOGGED, Boolean.valueOf(false)));
