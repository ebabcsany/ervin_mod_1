...
public class PoweredRailBlock extends net.minecraft.block.PoweredRailBlock {
   private final boolean isActivator;  // TRUE for an Activator Rail, FALSE for Powered Rail

   public PoweredRailBlock(Properties builder) {
      this(builder, false);
   }

   public PoweredRailBlock(Properties builder, boolean isPoweredRail) {
      super(builder, true);
      this.setDefaultState(this.stateContainer.getBaseState().with(SHAPE, RailShape.NORTH_SOUTH).with(POWERED, Boolean.FALSE));
   ...
   protected void updateState(BlockState state, World worldIn, BlockPos pos, Block blockIn) {
      ...
      if (flag1 != flag) {
         worldIn.setBlockState(pos, state.with(POWERED, flag1), 10);
