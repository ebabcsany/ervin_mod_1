...
public class Hurvruj extends RespawnAnchorBlock {
   ...
   private static boolean notFullyCharged(BlockState state) {
      return state.get(HURVRUJ_CHARGES) < 16;
   }
   ...
   public static void chargeHurvruj(World world, BlockPos pos, BlockState state) {
      world.setBlockState(pos, state.with(HURVRUJ_CHARGES, state.get(HURVRUJ_CHARGES) + 1), 15);
   ...
   public static int func_235565_a_(BlockState state, int charges) {
      return MathHelper.floor((float)(state.get(HURVRUJ_CHARGES)) / MAX_CHARGE * (float)charges);
   }
   ...
   public static int getChargeScale(BlockState state, int scale) {
      return MathHelper.floor((float)(state.get(HURVRUJ_CHARGES) - MIN_CHARGE) / MAX_CHARGE * (float)scale);
   }
