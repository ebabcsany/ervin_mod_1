...
public class ExampleNetherPortalBlock extends Block {
   public static final EnumProperty<Direction.Axis> AXIS = BlockStateProperties.HORIZONTAL_AXIS;
   protected static final VoxelShape X_AABB = Block.makeCuboidShape(0.0D, 0.0D, 6.0D, 16.0D, 16.0D, 10.0D);
   protected static final VoxelShape Z_AABB = Block.makeCuboidShape(6.0D, 0.0D, 0.0D, 10.0D, 16.0D, 16.0D);

   public ExampleNetherPortalBlock(Properties properties) {
      super(properties);
   ...
   public static boolean trySpawnPortal(IWorld world, BlockPos worldIn) {
      Size examplePortalBlock$size = isPortal(world, worldIn);
      if (examplePortalBlock$size != null && !ForgeEventFactory.onTrySpawnPortal(world, worldIn, examplePortalBlock$size)) {
   ...
   @Nullable
   public static Size isPortal(IWorld world, BlockPos worldIn) {
      Size examplePortalBlock$size = new Size(world, worldIn, Direction.Axis.X);
      if (examplePortalBlock$size.isValid() && examplePortalBlock$size.portalBlockCount == 0) {
         return examplePortalBlock$size;
      } else {
         Size examplePortalBlock$size1 = new Size(world, worldIn, Direction.Axis.Z);
   ...
   public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
      ...
      return !flag && !facingState.isIn(this) && !(new Size(worldIn, currentPos, direction$axis1)).validatePortal() ? Blocks.AIR.getDefaultState() : super.updatePostPlacement(stateIn, facing, facingState, worldIn, currentPos, facingPos);
   }
    ...
    * this method is unrelated to {@link #randomTick} and {@link #//randomTick(BlockState, ServerWorld, BlockPos, Random)}, and will always be called regardless
   ...
   public static BlockPattern.PatternHelper createPatternHelper(IWorld world, BlockPos worldIn) {
      ...
      Size netherportalblock$size = new Size(world, worldIn, Direction.Axis.X);
      ...
      if (!netherportalblock$size.isValid()) {
         ...
         netherportalblock$size = new Size(world, worldIn, Direction.Axis.Z);
      }
   ...
   public static class Size {
      ...
      protected int calculatePortalHeight() {
         ...
         for(this.height = 0; this.height < 21; ++this.height) {
            for(int i = 0; i < this.width; ++i) {
               BlockPos blockpos = Objects.requireNonNull(this.bottomLeft).offset(this.rightDir, i).up(this.height);
               BlockState blockstate = this.world.getBlockState(blockpos);
               if (!this.canConnect(blockstate)) {
                  break label56;
               }

               if (blockstate.isIn(BlockInit.EXAMPLE_PORTAL_BLOCK)) {
      ...
      protected boolean canConnect(BlockState pos) {
         return pos.isAir() || pos.isIn(BlockItemInit.AIR.get()) || pos.isIn(BlockInit.EXAMPLE_PORTAL_BLOCK);
      }
      ...
      public void placePortalBlocks() {
         for(int i = 0; i < this.width; ++i) {
            ...
            for(int j = 0; j < this.height; ++j) {
               this.world.setBlockState(blockpos.up(j), BlockInit.EXAMPLE_PORTAL_BLOCK.getDefaultState().with(NetherPortalBlock.AXIS, this.axis), 18);
            }
         }
