...
public abstract class JurkFluid extends FlowingFluid {
    ...
    public void animateTick(World worldIn, BlockPos pos, FluidState state, Random random) {
        if (!state.isSource() && !state.get(FALLING)) {
            ...
            worldIn.addParticle(ParticleTypes.LAVA, (double)pos.getX() + random.nextDouble(), (double)pos.getY() + random.nextDouble(), (double)pos.getZ() + random.nextDouble(), 0.0D, 0.0D, 0.0D);
        }
    ...
    {
        return FluidAttributes.builder(Ervin_mod_1.getKey("block/jurk/still"), Ervin_mod_1.getKey("block/jurk/flow"))
                .overlay(Ervin_mod_1.getKey("block/jurk/overlay"))
                .translationKey("block.ervin_mod_1.fluids.jurk")
                .color(0xFF7F21C2).build(this);
    }
