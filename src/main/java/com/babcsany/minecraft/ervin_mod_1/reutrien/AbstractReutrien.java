            ...
            for(int i = 0; i < this.spawnCount; ++i) {
               ...
               if (world.hasNoCollisions(optional.get().getBoundingBoxWithSizeApplied(d0, d1, d2)) && EntitySpawnPlacementRegistry.canSpawnEntity(optional.get(), world.getWorld(), SpawnReason.SPAWNER, new BlockPos(d0, d1, d2), world.getRandom())) {
                  ...
                  if (entity instanceof MobEntity) {
                     ...
                     if (!net.minecraftforge.event.ForgeEventFactory.canEntitySpawnReutrien(mobentity, world, (float)entity.getPosX(), (float)entity.getPosY(), (float)entity.getPosZ(), this)) {
                        continue;
                     }

                     if (this.spawnData.getNbt().size() == 1 && this.spawnData.getNbt().contains("id", 8)) {
                        if (!net.minecraftforge.event.ForgeEventFactory.doSpecialReutrienSpawn(mobentity, world, (float)entity.getPosX(), (float)entity.getPosY(), (float)entity.getPosZ(), this, SpawnReason.SPAWNER))
