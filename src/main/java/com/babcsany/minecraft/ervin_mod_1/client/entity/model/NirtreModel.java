...
@OnlyIn(Dist.CLIENT)
public class NirtreModel<T extends Entity> extends SegmentedModel<T> implements IHasHead, IHeadToggle {
   ...
   public NirtreModel(float scale) {
      this(scale, 64, 64);
   }

   public NirtreModel(float p_i51059_1_, int p_i51059_2_, int p_i51059_3_) {
      float f = 0.5F;
   ...
   public void setRotationAngles(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
      boolean flag = false;
      if (entityIn instanceof AbstractNirtreEntity) {
         flag = ((AbstractNirtreEntity)entityIn).getShakeHeadTicks() > 0;
      }
