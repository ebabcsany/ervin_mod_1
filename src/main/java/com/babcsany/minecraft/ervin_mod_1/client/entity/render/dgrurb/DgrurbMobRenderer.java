...
public abstract class DgrurbMobRenderer<T extends DgrurbMobEntity, M extends EntityModel<T>> extends LivingRenderer<T, M> {
   ...
   public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, DgrurbMobRenderer dgrurbMobRenderer) {
      super.render(entityIn, entityYaw, partialTicks, matrixStackIn, bufferIn, packedLightIn);
      Entity entity = entityIn.getLeashHolder();
      if (entity != null) {
         this.renderLeash(entityIn, partialTicks, matrixStackIn, bufferIn, entity, dgrurbMobRenderer);
      }
   }

   private <E extends Entity> void renderLeash(T entityLivingIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, E leashHolder, DgrurbMobRenderer dgrurbMobRenderer) {
      matrixStackIn.push();
      ...
      double d11 = MathHelper.lerp(partialTicks, entityLivingIn.prevPosY, entityLivingIn.getPosY()) + vector3d.y;
      ...
      int j = this.renderManager.getRenderer(leashHolder).getBlockLight(leashHolder, blockpos1, dgrurbMobRenderer);
