package com.babcsany.minecraft.ervin_mod_1.client.world;

import it.unimi.dsi.fastutil.objects.Object2ObjectArrayMap;
import it.unimi.dsi.fastutil.objects.Object2ObjectMap;
import net.minecraft.util.RegistryKey;
import net.minecraft.util.Util;
import net.minecraft.util.math.MathHelper;
import net.minecraft.util.math.vector.Vector3d;
import net.minecraft.world.DimensionType;
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.api.distmarker.OnlyIn;

import javax.annotation.Nullable;
import java.util.Optional;

@OnlyIn(Dist.CLIENT)
public abstract class ExampleDimensionRenderInfo {
   private static final Object2ObjectMap<RegistryKey<DimensionType>, ExampleDimensionRenderInfo> field_239208_a_ = Util.make(new Object2ObjectArrayMap<>(), (p_239214_0_) -> {
      Overworld dimensionrenderinfo$overworld = new Overworld();
      p_239214_0_.defaultReturnValue(dimensionrenderinfo$overworld);
      p_239214_0_.put(DimensionType.OVERWORLD, dimensionrenderinfo$overworld);
      p_239214_0_.put(DimensionType.EXAMPLE_OVERWORLD, new ExampleOverworld());
      p_239214_0_.put(DimensionType.THE_NETHER, new Nether());
      p_239214_0_.put(DimensionType.THE_END, new End());
   });
   private final float[] field_239209_b_ = new float[4];
   private final float field_239210_c_;
   private final boolean field_239211_d_;
   private final FogType field_241680_e_;
   private final boolean field_241681_f_;
   private final boolean field_239212_e_;

   public ExampleDimensionRenderInfo(float p_i241259_1_, boolean p_i241259_2_, FogType p_i241259_3_, boolean p_i241259_4_, boolean p_i241259_5_) {
      this.field_239210_c_ = p_i241259_1_;
      this.field_239211_d_ = p_i241259_2_;
      this.field_241680_e_ = p_i241259_3_;
      this.field_241681_f_ = p_i241259_4_;
      this.field_239212_e_ = p_i241259_5_;
   }

   public static ExampleDimensionRenderInfo func_239215_a_(Optional<RegistryKey<DimensionType>> p_239215_0_) {
      return field_239208_a_.get(p_239215_0_.orElse(DimensionType.OVERWORLD));
   }

   @Nullable
   public float[] func_230492_a_(float p_230492_1_, float p_230492_2_) {
      float f = 0.4F;
      float f1 = MathHelper.cos(p_230492_1_ * ((float)Math.PI * 2F)) - 0.0F;
      float f2 = -0.0F;
      if (f1 >= -0.4F && f1 <= 0.4F) {
         float f3 = (f1 - -0.0F) / 0.4F * 0.5F + 0.5F;
         float f4 = 1.0F - (1.0F - MathHelper.sin(f3 * (float)Math.PI)) * 0.99F;
         f4 = f4 * f4;
         this.field_239209_b_[0] = f3 * 0.3F + 0.7F;
         this.field_239209_b_[1] = f3 * f3 * 0.7F + 0.2F;
         this.field_239209_b_[2] = f3 * f3 * 0.0F + 0.2F;
         this.field_239209_b_[3] = f4;
         return this.field_239209_b_;
      } else {
         return null;
      }
   }

   public float func_239213_a_() {
      return this.field_239210_c_;
   }

   public boolean func_239216_b_() {
      return this.field_239211_d_;
   }

   public abstract Vector3d func_230494_a_(Vector3d p_230494_1_, float p_230494_2_);

   public abstract boolean func_230493_a_(int p_230493_1_, int p_230493_2_);

   public FogType func_241683_c_() {
      return this.field_241680_e_;
   }

   public boolean func_241684_d_() {
      return this.field_241681_f_;
   }

   public boolean func_239217_c_() {
      return this.field_239212_e_;
   }

   @OnlyIn(Dist.CLIENT)
   public static class End extends ExampleDimensionRenderInfo {
      public End() {
         super(Float.NaN, false, FogType.END, true, false);
      }

      public Vector3d func_230494_a_(Vector3d p_230494_1_, float p_230494_2_) {
         return p_230494_1_.scale((double)0.15F);
      }

      public boolean func_230493_a_(int p_230493_1_, int p_230493_2_) {
         return false;
      }

      @Nullable
      public float[] func_230492_a_(float p_230492_1_, float p_230492_2_) {
         return null;
      }
   }

   @OnlyIn(Dist.CLIENT)
   public static enum FogType {
      NONE,
      NORMAL,
      NORMAL1,
      END;
   }

   @OnlyIn(Dist.CLIENT)
   public static class Nether extends ExampleDimensionRenderInfo {
      public Nether() {
         super(Float.NaN, true, FogType.NONE, false, true);
      }

      public Vector3d func_230494_a_(Vector3d p_230494_1_, float p_230494_2_) {
         return p_230494_1_;
      }

      public boolean func_230493_a_(int p_230493_1_, int p_230493_2_) {
         return true;
      }
   }

   @OnlyIn(Dist.CLIENT)
   public static class ExampleOverworld extends ExampleDimensionRenderInfo {
      public ExampleOverworld() {
         super(Float.NaN, true, FogType.NORMAL1, true, true);
      }

      public Vector3d func_230494_a_(Vector3d p_230494_1_, float p_230494_2_) {
         return p_230494_1_;
      }

      public boolean func_230493_a_(int p_230493_1_, int p_230493_2_) {
         return true;
      }
   }

   @OnlyIn(Dist.CLIENT)
   public static class Overworld extends ExampleDimensionRenderInfo {
      public Overworld() {
         super(128.0F, true, FogType.NORMAL, false, false);
      }

      public Vector3d func_230494_a_(Vector3d p_230494_1_, float p_230494_2_) {
         return p_230494_1_.mul((double)(p_230494_2_ * 0.94F + 0.06F), (double)(p_230494_2_ * 0.94F + 0.06F), (double)(p_230494_2_ * 0.91F + 0.09F));
      }

      public boolean func_230493_a_(int p_230493_1_, int p_230493_2_) {
         return false;
      }
   }
}