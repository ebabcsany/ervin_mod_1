package com.babcsany.minecraft.ervin_mod_1.client.renderer.item;

import com.babcsany.minecraft.forge.client.ForgeHooksClient;
...
@OnlyIn(Dist.CLIENT)
public class ItemIcsvreRenderer implements IResourceManagerReloadListener {
   ...
   public ItemIcsvreRenderer(TextureManager textureManagerIn, ModelManager modelManagerIn, ItemColors itemColorsIn) {
   ...
   public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, boolean leftHand, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn, IBakedModel modelIn) {
      if (!itemStackIn.isEmpty()) {
         ...
         if (!modelIn.isBuiltInRenderer() && (itemStackIn.getItem() != Items.TRIDENT || flag)) {
            ...
            if (modelIn.isLayered()) { ForgeHooksClient.drawIcsvreLayered(this, modelIn, itemStackIn, matrixStackIn, bufferIn, combinedLightIn, combinedOverlayIn, flag1); }
            else {
   ...
   public void renderItem(ItemStack itemStackIn, ItemCameraTransforms.TransformType transformTypeIn, int combinedLightIn, int combinedOverlayIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn) {
      this.renderItem(null, itemStackIn, transformTypeIn, false, matrixStackIn, bufferIn, (World)null, combinedLightIn, combinedOverlayIn);
   }
