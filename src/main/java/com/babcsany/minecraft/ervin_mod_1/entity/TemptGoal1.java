...
public class TemptGoal1 extends Goal {
   ...
   public TemptGoal1(CreatureEntity creature1In, double speedIn, boolean scaredByPlayerMovementIn, Ingredient temptItemsIn) {
      ...
      if (!(creature1In.getNavigator() instanceof GroundPathNavigator) && !(creature1In.getNavigator() instanceof FlyingPathNavigator)) {
   ...
   public boolean shouldExecute() {
      if (this.delayTemptCounter > 0) {
         ...
         this.closestPlayer = this.creature1.world.getClosestPlayer(ENTITY_PREDICATE, this.creature1);
   ...
   public boolean shouldContinueExecuting() {
      if (this.isScaredByPlayerMovement()) {
         if (this.creature1.getDistanceSq(this.closestPlayer) < 36.0D) {
   ...
   public void resetTask() {
      ...
      this.creature1.getNavigator().clearPath();
    ...
    */
   public void tick() {
      this.creature1.getLookController().setLookPositionWithEntity(this.closestPlayer, (float)(this.creature1.getHorizontalFaceSpeed() + 20), (float)this.creature1.getVerticalFaceSpeed());
      if (this.creature1.getDistanceSq(this.closestPlayer) < 6.25D) {
         this.creature1.getNavigator().clearPath();
      } else {
         this.creature1.getNavigator().tryMoveToEntityLiving(this.closestPlayer, this.speed);
      }
