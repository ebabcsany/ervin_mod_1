...
public class ZurInventory implements IInventory, INameable {
      ...
      for(NonNullList<ItemStack> nonnulllist : this.allInventories) {
         for(int i = 0; i < nonnulllist.size(); ++i) {
            if (!nonnulllist.get(i).isEmpty()) {
               nonnulllist.get(i).inventoryTick(this.zur.world, this.zur, i, this.currentItem == i);
            }
         }
      }
      armorInventory.forEach(e -> e.onArmorTick(zur.world, player));
   }
         ...
         try {
            if (stack.isDamaged()) {
               ...
               if (slotIn >= 0) {
                  ...
                  stack.setCount(0);
                  return true;
               } else if (this.zur.world.isRemote) {
                  stack.setCount(0);
                  return true;
               } else {
                  return false;
               ...
               if (stack.getCount() == i && this.zur.world.isRemote) {
                  stack.setCount(0);
                  return true;
               } else {
      ...
      if (!(p_234563_2_ <= 0.0F)) {
         ...
         for(int i = 0; i < this.armorInventory.size(); ++i) {
            ...
            if ((!p_234563_1_.isFireDamage() || !itemstack.getItem().isBurnable()) && itemstack.getItem() instanceof ArmorItem) {
               int j = i;
               itemstack.damageItem((int)p_234563_2_, this.zur, (p_214023_1_) -> {
   ...
   public void dropAllItems() {
      for(List<ItemStack> list : this.allInventories) {
         for(int i = 0; i < list.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
               this.zur.dropItem(itemstack, true, false);
    ...
    */
   public boolean isUsableByPlayer(AbstractZurEntity zur) {
      if (this.zur.removed) {
         return false;
      } else {
         return !(zur.getDistanceSq(this.zur) > 2048.0D);
      }
   }
    ...
    */
   public void copyInventory(ZurInventory playerInventory) {
