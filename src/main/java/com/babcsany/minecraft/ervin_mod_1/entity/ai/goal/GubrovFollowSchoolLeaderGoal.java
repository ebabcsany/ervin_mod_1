...
public class GubrovFollowSchoolLeaderGoal extends Goal {
   private final AbstractGroupGubrovEntity taskOwner;
   private int navigateTimer;
   private int cooldown;

   public GubrovFollowSchoolLeaderGoal(AbstractGroupGubrovEntity taskOwnerIn) {
      this.taskOwner = taskOwnerIn;
      this.cooldown = this.getNewCooldown(taskOwnerIn);
   }

   protected int getNewCooldown(AbstractGroupGubrovEntity taskOwnerIn) {
   ...
   public boolean shouldExecute() {
      if (this.taskOwner.isGroupLeader()) {
         ...
         Predicate<AbstractGroupGubrovEntity> predicate = (fish) -> {
