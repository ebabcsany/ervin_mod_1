...
public abstract class AgeableEntity1 extends CreatureEntity1 {
   private static final DataParameter<Boolean> BABY = EntityDataManager.createKey(AgeableEntity1.class, DataSerializers.BOOLEAN);
   protected int growingAge;
   protected int forcedAge;
   protected int forcedAgeTimer;

   protected AgeableEntity1(EntityType<? extends AgeableEntity1> type, World worldIn) {
      super(type, worldIn);
   }

   public ILivingEntityData onInitialSpawn(IWorld worldIn, DifficultyInstance difficultyIn, SpawnReason reason, @Nullable ILivingEntityData spawnDataIn, @Nullable CompoundNBT dataTag) {
      if (spawnDataIn == null) {
         spawnDataIn = new AgeableEntity1.AgeableData();
      }

      AgeableEntity1.AgeableData ageableentity$ageabledata = (AgeableEntity1.AgeableData)spawnDataIn;
   ...
   @Nullable
   public abstract AgeableEntity1 createChild(AgeableEntity1 ageable);

   protected void onChildSpawnFromEgg(PlayerEntity playerIn, AgeableEntity1 child) {
   }
   ...
   public boolean processInteract(PlayerEntity player, Hand hand) {
      ...
      if (item instanceof SpawnEggItem && ((SpawnEggItem)item).hasType(itemstack.getTag(), this.getType())) {
         if (!this.world.isRemote) {
            AgeableEntity1 monsterentity = this.createChild(this);
            if (monsterentity != null) {
               monsterentity.setGrowingAge(-24000);
               monsterentity.setLocationAndAngles(this.getPosX(), this.getPosY(), this.getPosZ(), 0.0F, 0.0F);
               this.world.addEntity(monsterentity);
               if (itemstack.hasDisplayName()) {
                  monsterentity.setCustomName(itemstack.getDisplayName());
               }

               this.onChildSpawnFromEgg(player, monsterentity);
