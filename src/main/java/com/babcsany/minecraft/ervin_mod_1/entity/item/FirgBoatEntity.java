...
public class FirgBoatEntity extends Entity {
   ...
   private static final DataParameter<Boolean> field_199704_e = EntityDataManager.createKey(FirgBoatEntity.class, DataSerializers.BOOLEAN);
   private static final DataParameter<Boolean> field_199705_f = EntityDataManager.createKey(FirgBoatEntity.class, DataSerializers.BOOLEAN);
   ...
   public FirgBoatEntity(EntityType<BoatEntity> p_i50129_1_, World p_i50129_2_) {
   ...
   protected void registerData() {
      ...
      this.dataManager.register(DAMAGE_TAKEN, 10.0F);
      this.dataManager.register(BOAT_TYPE, Type1.FIRG.ordinal());
   ...
   private Status getBoatStatus() {
      Status boat_entity$status = this.getUnderwaterStatus();
      if (boat_entity$status != null) {
         this.waterLevel = this.getBoundingBox().maxY;
         return boat_entity$status;
               ...
               for(int i2 = i1; i2 < j1; ++i2) {
                  ...
                  if (ifluidstate.isTagged(FluidTags.WATER) && d0 < (double)((float)blockpos$pooledmutable.getY() + ifluidstate.getActualHeight(this.world, blockpos$pooledmutable))) {
                     if (!ifluidstate.isSource()) {
                        Status boatentity$status = Status.UNDER_FLOWING_WATER;
   ...
   protected void readAdditional(CompoundNBT compound) {
      if (compound.contains("Type1", 8)) {
         this.setModBoatType(Type1.getTypeFromString(compound.getString("Type")));
      }
   ...
   public void setModBoatType(Type1 boatType) {
      this.dataManager.set(BOAT_TYPE, boatType.ordinal());
   }

   public Type1 getBoatType() {
      return Type1.byId(this.dataManager.get(BOAT_TYPE));
   }
   ...
   public static enum Type1 {
      ...
      private Type1(Block block, String name) {
         this.name = name;
         this.block = block;
      }
       ...
       */
      public static Type1 byId(int id) {
         Type1[] aboatentity$type = values();
      ...
      public static Type1 getTypeFromString(String nameIn) {
         Type1[] aboatentity$type = values();
