...
public class BabyEntitySpawnEvent extends net.minecraftforge.eventbus.api.Event
{
    ...
    private DgrurbMobEntity dgrurbMobEntity;
    private DgrurbAgeableEntity dgrurbAgeableEntity;

    public BabyEntitySpawnEvent(LivingEntity parentA, LivingEntity parentB, @Nullable AbstractkZurEntity proposedChild)
    {
    ...
    public BabyEntitySpawnEvent(DgrurbMobEntity parentA, DgrurbMobEntity parentB, @Nullable DgrurbAgeableEntity proposedChild)
    {
        //causedByPlayer calculated here to simplify the patch.
        PlayerEntity causedByPlayer = null;
        if (parentA instanceof AnimalDgrurbEntity) {
            causedByPlayer = ((AnimalDgrurbEntity)parentA).getLoveCause();
        }

        if (causedByPlayer == null && parentB instanceof AnimalDgrurbEntity)
        {
            causedByPlayer = ((AnimalDgrurbEntity)parentB).getLoveCause();
        }

        this.parentA = parentA;
        this.parentB = parentB;
        this.causedByPlayer = causedByPlayer;
        this.dgrurbAgeableEntity = proposedChild;
    }
    ...
    @Nullable
    public DgrurbAgeableEntity getDgrurbAnimalChild()
    {
        return dgrurbAgeableEntity;
    }
