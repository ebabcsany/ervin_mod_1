...
public class HuihkEntity extends ProjectileEntity {
   ...
   private float huihkApproachAngle;
   private boolean field_234595_aq_ = true;
   private Entity caughtEntity;
   private HuihkEntity.State currentState = HuihkEntity.State.FLYING;
   private int luck;
   private int lureSpeed;

   public HuihkEntity(EntityType<? extends ProjectileEntity> p_i231584_1_, World p_i231584_2_) {
      super(p_i231584_1_, p_i231584_2_);
   }

   /*public HuihkEntity(World p_i50219_1_, PlayerEntity p_i50219_2_, int p_i50219_3_, int p_i50219_4_) {
      super(EntityInit.HUIHK, p_i50219_1_);
   ...
   @OnlyIn(Dist.CLIENT)
   public HuihkEntity(World worldIn, PlayerEntity p_i47290_2_, double x, double y, double z) {
   ...
   public void tick() {
      ...
      if (playerentity == null) {
         ...
         boolean flag = f > 0.0F;
         if (this.currentState == HuihkEntity.State.FLYING) {
            if (this.caughtEntity != null) {
               this.setMotion(Vector3d.ZERO);
               this.currentState = HuihkEntity.State.HOOKED_IN_ENTITY;
               return;
            }

            if (flag) {
               this.setMotion(this.getMotion().mul(0.3D, 0.2D, 0.3D));
               this.currentState = HuihkEntity.State.BOBBING;
               return;
            }

            this.checkCollision();
         } else {
            if (this.currentState == HuihkEntity.State.HOOKED_IN_ENTITY) {
               if (this.caughtEntity != null) {
                  if (this.caughtEntity.removed) {
                     this.caughtEntity = null;
                     this.currentState = HuihkEntity.State.FLYING;
                  } else {
            ...
            if (this.currentState == HuihkEntity.State.BOBBING) {
         ...
         this.func_234617_x_();
         if (this.currentState == HuihkEntity.State.FLYING && (this.onGround || this.collidedHorizontally)) {
            this.setMotion(Vector3d.ZERO);
         }
   ...
   private void catchingFish(BlockPos p_190621_1_) {
      ...
      if (this.ticksCatchable > 0) {
         ...
         if (this.ticksCatchableDelay > 0) {
            this.huihkApproachAngle = (float)((double)this.huihkApproachAngle + this.rand.nextGaussian() * 4.0D);
            float f = this.huihkApproachAngle * ((float)Math.PI / 180F);
            ...
            if (serverworld.getBlockState(new BlockPos((int)d0, (int)d1 - 1, (int)d2)).getMaterial() == Material.LAVA) {
               if (this.rand.nextFloat() < 0.15F) {
                  serverworld.spawnParticle(ParticleTypes.FLAME, d0, d1 - (double)0.1F, d2, 1, f1, 0.1D, f2, 0.0D);
               }

               float f3 = f1 * 0.04F;
               float f4 = f2 * 0.04F;
               serverworld.spawnParticle(ParticleTypes.LANDING_LAVA, d0, d1, d2, 0, f4, 0.01D, -f3, 1.0D);
               serverworld.spawnParticle(ParticleTypes.LANDING_LAVA, d0, d1, d2, 0, -f4, 0.01D, f3, 1.0D);
            }
         } else {
            ...
            serverworld.spawnParticle(ParticleTypes.FLAME, this.getPosX(), d3, this.getPosZ(), (int)(1.0F + this.getWidth() * 20.0F), (double)this.getWidth(), 0.0D, (double)this.getWidth(), (double)0.2F);
         ...
         if (this.rand.nextFloat() < f5) {
            ...
            if (serverworld.getBlockState(new BlockPos(d4, d5 - 1.0D, d6)).getMaterial() == Material.LAVA) {
               serverworld.spawnParticle(ParticleTypes.LAVA, d4, d5, d6, 2 + this.rand.nextInt(2), 0.1F, 0.0D, 0.1F, 0.0D);
            }
         }

         if (this.ticksCaughtDelay <= 0) {
            this.huihkApproachAngle = MathHelper.nextFloat(this.rand, 0.0F, 360.0F);
   ...
   private boolean func_234603_b_(BlockPos p_234603_1_) {
      HuihkEntity.LavaType huihkEntity$lavaType = HuihkEntity.LavaType.INVALID;

      for(int i = -1; i <= 2; ++i) {
         HuihkEntity.LavaType huihkEntity$lavaType1 = this.func_234602_a_(p_234603_1_.add(-2, i, -2), p_234603_1_.add(2, i, 2));
         switch(huihkEntity$lavaType1) {
         case INVALID:
            return false;
         case ABOVE_LAVA:
            if (huihkEntity$lavaType == HuihkEntity.LavaType.INVALID) {
               return false;
            }
            break;
         case INSIDE_LAVA:
            if (huihkEntity$lavaType == LavaType.ABOVE_LAVA) {
               return false;
            }
         }

         huihkEntity$lavaType = huihkEntity$lavaType1;
      }

      return true;
   }

   private HuihkEntity.LavaType func_234602_a_(BlockPos p_234602_1_, BlockPos p_234602_2_) {
      return BlockPos.getAllInBox(p_234602_1_, p_234602_2_).map(this::func_234604_c_).reduce((p_234601_0_, p_234601_1_) -> p_234601_0_ == p_234601_1_ ? p_234601_0_ : HuihkEntity.LavaType.INVALID).orElse(HuihkEntity.LavaType.INVALID);
   }

   private HuihkEntity.LavaType func_234604_c_(BlockPos p_234604_1_) {
      BlockState blockstate = this.world.getBlockState(p_234604_1_);
      if (!blockstate.isAir() && !blockstate.isIn(Blocks.LILY_PAD)) {
         FluidState fluidstate = blockstate.getFluidState();
         return fluidstate.isTagged(FluidTags.LAVA) && fluidstate.isSource() && blockstate.getCollisionShape(this.world, p_234604_1_).isEmpty() ? LavaType.INSIDE_LAVA : HuihkEntity.LavaType.INVALID;
      } else {
   ...
   public int handleHookRetraction(ItemStack p_146034_1_) {
      ...
      if (!this.world.isRemote && playerentity != null) {
         int i = 0;
         com.babcsany.minecraft.ervin_mod_1.entity.event.ItemHuihkedEvent event = null;
         if (this.caughtEntity != null) {
            this.bringInHookedEntity();
            CriteriaTriggers1.HUIHK_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, p_146034_1_, this, Collections.emptyList());
            ...
            event = new com.babcsany.minecraft.ervin_mod_1.entity.event.ItemHuihkedEvent(list, this.onGround ? 2 : 1, this);
            net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(event);
            if (event.isCanceled()) {
               this.remove();
               return event.getRodDamage();
            }
            CriteriaTriggers1.HUIHK_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, p_146034_1_, this, list);
