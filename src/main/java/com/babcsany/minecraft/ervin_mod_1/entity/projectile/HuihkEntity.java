...
public class HuihkEntity extends FishingBobberEntity {
        ...
        if (this.ticksCatchable > 0) {
            ...
            if (this.ticksCatchableDelay > 0) {
                ...
                if (serverworld.getBlockState(new BlockPos((int)d0, (int)d1 - 1, (int)d2)).getMaterial() == Material.LAVA) {
                    if (this.rand.nextFloat() < 0.15F) {
                        serverworld.spawnParticle(ParticleTypes.DRIPPING_LAVA, d0, d1 - (double)0.1F, d2, 1, (double)f1, 0.1D, (double)f2, 0.0D);
                    }
                ...
                serverworld.spawnParticle(ParticleTypes.DRIPPING_LAVA, this.getPosX(), d3, this.getPosZ(), (int)(1.0F + this.getWidth() * 20.0F), (double)this.getWidth(), 0.0D, (double)this.getWidth(), (double)0.2F);
            ...
            if (this.rand.nextFloat() < f5) {
                ...
                if (serverworld.getBlockState(new BlockPos(d4, d5 - 1.0D, d6)).getMaterial() == Material.LAVA) {
                    serverworld.spawnParticle(ParticleTypes.LANDING_LAVA, d4, d5, d6, 2 + this.rand.nextInt(2), (double)0.1F, 0.0D, (double)0.1F, 0.0D);
                }
            }
    ...
    private WaterType func_234604_c_(BlockPos p_234604_1_) {
        ...
        if (!blockstate.isAir() && !blockstate.matchesBlock(Blocks.LILY_PAD)) {
            ...
            return fluidstate.isTagged(FluidTags.LAVA) && fluidstate.isSource() && blockstate.getCollisionShapeUncached(this.world, p_234604_1_).isEmpty() ? WaterType.INSIDE_WATER : WaterType.INVALID;
        } else {
    ...
    public int handleHookRetraction(ItemStack p_146034_1_) {
        ...
        if (!this.world.isRemote && playerentity != null) {
            ...
            if (this.caughtEntity != null) {
                ...
                LootTable loottable = this.world.getServer().getLootTableManager().getLootTableFromLocation(ModLootTables.GAMEPLAY_HUIHK);
                ...
                ModCriteriaTriggers.HUIHK_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, p_146034_1_, this, list);
