 ...
 **/
@Cancelable
public class HhijBabyEntitySpawnEvent extends net.minecraftforge.eventbus.api.Event
{
    private final MobEntity parentA;
    private final MobEntity parentB;
    private final PlayerEntity causedByPlayer;
    private HhijAgeableEntity child;

    public HhijBabyEntitySpawnEvent(MobEntity parentA, MobEntity parentB, @Nullable HhijAgeableEntity proposedChild)
    {
        //causedByPlayer calculated here to simplify the patch.
        PlayerEntity causedByPlayer = null;
        if (parentA instanceof HhijAnimalEntity) {
            causedByPlayer = ((HhijAnimalEntity)parentA).getLoveCause();
        }

        if (causedByPlayer == null && parentB instanceof HhijAnimalEntity)
        {
            causedByPlayer = ((HhijAnimalEntity)parentB).getLoveCause();
        }
    ...
    @Nullable
    public HhijAgeableEntity getChild()
    {
        return child;
    }

    public void setChild(HhijAgeableEntity proposedChild)
    {
        child = proposedChild;
    }
}