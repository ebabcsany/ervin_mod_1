 ...
 **/
@Cancelable
public class BabyEntitySpawnEvent1 extends net.minecraftforge.eventbus.api.Event
{
    private final MobEntity parentA;
    private final MobEntity parentB;
    private final PlayerEntity causedByPlayer;
    private AgeableEntity1 child;

    public BabyEntitySpawnEvent1(MobEntity parentA, MobEntity parentB, @Nullable AgeableEntity1 proposedChild)
    {
        //causedByPlayer calculated here to simplify the patch.
        PlayerEntity causedByPlayer = null;
        if (parentA instanceof AnimalEntity1) {
            causedByPlayer = ((AnimalEntity1)parentA).getLoveCause();
        }

        if (causedByPlayer == null && parentB instanceof AnimalEntity1)
        {
            causedByPlayer = ((AnimalEntity1)parentB).getLoveCause();
        }
    ...
    @Nullable
    public AgeableEntity1 getChild()
    {
        return child;
    }

    public void setChild(AgeableEntity1 proposedChild)
    {
        child = proposedChild;
    }
}