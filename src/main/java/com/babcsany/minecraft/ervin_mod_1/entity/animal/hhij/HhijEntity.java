...
public class HhijEntity extends HhijTameableEntity implements IAngerable {
    ...
    public static final Predicate<LivingEntity> TARGET_ENTITIES = (living) -> {
        EntityType<?> entitytype = living.getType();
        return entitytype == EntityType.SHEEP || entitytype == EntityType.RABBIT || entitytype == EntityType.FOX;
    };
    public static final Map<Item, Integer> FOOD_VALUES = itemIntegerMapOf(FoodItemInit.COOKED_STEEF.get(), 4, FoodItemInit.STEEF.get(), 3, isBurnableFoodItemInit.TIRKS.get(), 20, isBurnableFoodItemInit.VIRK.get(), 50, FoodItemInit.VOLMINT.get(), 1, FoodItemInit.MRONT.get(), 10, FoodItemInit.COOKED_MRONT.get(), 6);
    ...
    private float timeHhijIsShaking;
    private float prevTimeHhijIsShaking;
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(5, new HhijMeleeAttackGoal(this, 1.0, true));
        this.goalSelector.addGoal(6, new HhijFollowOwnerGoal(this, 1.0, 10.0F, 2.0F, false));
        this.goalSelector.addGoal(7, new HhijBreedGoal(this, 1.0));
        this.goalSelector.addGoal(8, new HhijWaterAvoidingRandomWalkingGoal(this, 1.0));
        ...
        this.targetSelector.addGoal(3, (new HhijHurtByTargetGoal(this, WolfEntity.class)).setCallsForHelp(WolfEntity.class, ZombifiedPiglinEntity.class, AbstractZurEntity.class));
        this.targetSelector.addGoal(4, new NearestAttackableTargetGoal<>(this, PlayerEntity.class, 10, true, false, this::func_233680_b_));
        this.targetSelector.addGoal(5, new HhijNonTamedTargetGoal<>(this, MonsterEntity.class, false, TARGET_ENTITIES));
        this.targetSelector.addGoal(6, new HhijNonTamedTargetGoal<>(this, PatrollerEntity.class, false, TurtleEntity.TARGET_DRY_BABY));
    ...
    public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
        return MobEntity.func_233666_p_()
                ...
                .createMutableAttribute(Attributes.MAX_HEALTH, 20000.0D)
                .createMutableAttribute(Attributes.ATTACK_DAMAGE, 5000.0D);
    }
    ...
    public void readAdditional(CompoundNBT nbt) {
        ...
        this.readAngerNBT((ServerWorld) this.world, nbt);
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (this.isAngry()) {
            ...
            return this.isTamed() && this.getHealth() < 30000.0F ? SoundEvents.ENTITY_WOLF_WHINE : SoundEvents.ENTITY_WOLF_PANT;
        }
    }

    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        SoundEvent sound = SoundEvents.ENTITY_GENERIC_HURT;
        if (attackingPlayer != null && attackEntityAsMob(attackingPlayer)) {
            sound = SoundEvents.ENTITY_WOLF_HURT;
        }
        return sound;
    }

    protected SoundEvent getDeathSound() {
        SoundEvent sound = SoundEvents.ENTITY_GENERIC_DEATH;
        if (attackingPlayer != null && attackEntityAsMob(attackingPlayer)) {
            sound = SoundEvents.ENTITY_WOLF_DEATH;
        }
        return sound;
    }
   ...
   protected float getSoundVolume() {
    ...
    public void livingTick() {
        ...
        if (!this.world.isRemote && this.isWet && !this.isShaking && !this.hasPath() && this.onGround) {
            this.isShaking = true;
            this.timeHhijIsShaking = 0.0F;
            this.prevTimeHhijIsShaking = 0.0F;
    ...
    public void tick() {
        ...
        if (this.isAlive()) {
            ...
            if (this.isInWaterRainOrBubbleColumn()) {
                ...
                if (this.timeHhijIsShaking == 0.0F) {
                    this.playSound(SoundEvents.ENTITY_WOLF_SHAKE, this.getSoundVolume(), (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F);
                }

                this.prevTimeHhijIsShaking = this.timeHhijIsShaking;
                this.timeHhijIsShaking += 0.05F;
                if (this.prevTimeHhijIsShaking >= 2.0F) {
                    this.isWet = false;
                ...
                if (this.timeHhijIsShaking > 0.4F) {
                    float f = (float) this.getPosY();
                    int i = (int) (MathHelper.sin((this.timeHhijIsShaking - 0.4F) * (float) Math.PI) * 7.0F);
    ...
    /**
     * Used when calculating the amount of shading to apply while the hhij is wet.
     */
    @OnlyIn(Dist.CLIENT)
    public float getShadingWhileWet(float partialTicks) {
        return Math.min(0.5F + MathHelper.lerp(partialTicks, this.prevTimeHhijIsShaking, this.timeHhijIsShaking) / 2.0F * 0.5F, 1.0F);
    }

    @OnlyIn(Dist.CLIENT)
    public float getShakeAngle(float partialTicks, float offset) {
        float f = (MathHelper.lerp(partialTicks, this.prevTimeHhijIsShaking, this.timeHhijIsShaking) + offset) / 1.8F;
        if (f < 0.0F) {
            f = 0.0F;
        } else if (f > 1.0F) {
            f = 1.0F;
    ...
    public void setTamed(boolean tamed) {
        ...
        if (tamed) {
            Objects.requireNonNull(this.getAttribute(Attributes.MAX_HEALTH)).setBaseValue(20000.0D);
            this.setHealth(20000.0F);
        } else {
            Objects.requireNonNull(this.getAttribute(Attributes.MAX_HEALTH)).setBaseValue(8000.0D);
        }

        Objects.requireNonNull(this.getAttribute(Attributes.ATTACK_DAMAGE)).setBaseValue(5000.0D);
    }
    ...
    public ActionResultType getEntityInteractionResult(PlayerEntity player, Hand hand) {
        ...
        if (this.world.isRemote) {
            ...
            if (this.isTamed()) {
                ...
                if (this.rand.nextInt(3) == 0 && !onAnimalTame(this, player)) {
    ...
    public HhijEntity createChild(ServerWorld serverWorld, HhijAgeableEntity ageable) {
        HhijEntity hhij = EntityInit.HHIJ.get().create(serverWorld);
        UUID uuid = this.getOwnerId();
        if (uuid != null) {
            hhij.setOwnerId(uuid);
            hhij.setTamed(true);
        }
    ...
    public boolean canMateWith(HhijAnimalEntity otherAnimal) {
        if (otherAnimal == this) {
            ...
            HhijEntity hhij = (HhijEntity) otherAnimal;
            if (!hhij.isTamed()) {
                return false;
            } else if (hhij.isEntitySleeping()) {
                return false;
            } else {
                return this.isInLove() && hhij.isInLove();
            }
        }
    }
    ...
    public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
        if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
            if (target instanceof HhijEntity) {
                HhijEntity hhij = (HhijEntity) target;
                return !hhij.isTamed() || hhij.getOwner() != owner;
