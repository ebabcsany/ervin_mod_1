package com.babcsany.minecraft.ervin_mod_1.container.trader;
...
public class TraderBlockContainer extends Container {
   ...
   public TraderBlockContainer(int id, PlayerInventory playerInventoryIn) {
      this(id, playerInventoryIn, new NPCMerchant(playerInventoryIn.player));
   }

   public TraderBlockContainer(int id, PlayerInventory playerInventoryIn, IMerchant merchantIn) {
      super(ContainerInit.TRADER_BLOCK.get(), id);
      this.merchant = merchantIn;
      this.merchantInventory = new TraderBlockInventory(merchantIn);
      this.addSlot(new Slot(this.merchantInventory, 0, 136, 37));
      this.addSlot(new Slot(this.merchantInventory, 1, 162, 37));
      this.addSlot(new TraderBlockResultSlot(playerInventoryIn.player, merchantIn, this.merchantInventory, 2, 220, 37));
   ...
   public TraderBlockContainer(int id, PlayerInventory playerInventoryIn, PacketBuffer packetBuffer) {
      this(id, playerInventoryIn, new NPCMerchant(playerInventoryIn.player));
   }
   ...
   protected void addTrades(MerchantOffers givenMerchantOffers, TraderBlockTrades.ITrade[] newTrades, int maxNumbers) {
      ...
      for(Integer integer : set) {
         TraderBlockTrades.ITrade $tradertrades$itrade = newTrades[integer];
         MerchantOffer merchantoffer = $tradertrades$itrade.getOffer(this, rand);
   ...
   protected void populateTradeData() {
      TraderBlockTrades.ITrade[] a$tradertrades$itrade = TraderBlockTrades.field_221240_b.get(1);
      final Random rand = new Random();
      if (a$tradertrades$itrade != null) {
         MerchantOffers merchantoffers = this.getOffers();
         this.addTrades(merchantoffers, a$tradertrades$itrade, 10);
         int i = rand.nextInt(a$tradertrades$itrade.length);
         TraderBlockTrades.ITrade $tradertrades$itrade = a$tradertrades$itrade[i];
         MerchantOffer merchantoffer = $tradertrades$itrade.getOffer(this, rand);
         if (merchantoffer != null) {
