...
public class Icsvre extends ItemEntity {
    private static final DataParameter<ItemStack> ITEM = EntityDataManager.createKey(Icsvre.class, DataSerializers.ITEMSTACK);
    private int age;
    ...
    public Icsvre(EntityType<? extends Icsvre> p_i50217_1_, World world) {
        super(p_i50217_1_, world);
        this.hoverStart = (float)(Math.random() * Math.PI * 2.0D);
    }

    public Icsvre(World worldIn, double x, double y, double z) {
    ...
    public Icsvre(World worldIn, double x, double y, double z, ItemStack stack) {
        this(worldIn, x, y, z);
        this.setItem(stack);
        stack.getItem();
    ...
    private void searchForOtherItemsNearby() {
        if (this.func_213857_z()) {
            for(Icsvre icsvre : this.world.getEntitiesWithinAABB(Icsvre.class, this.getBoundingBox().grow(0.5D, 0.0D, 0.5D), (icsvre) -> icsvre != this && icsvre.func_213857_z())) {
    ...
    private void func_226530_a_(Icsvre item) {
        ItemStack itemstack = this.getItem();
    ...
    private static void func_226531_a_(Icsvre entity, ItemStack stack1, ItemStack stack2) {
        ItemStack itemstack = mergeStacks(stack1, stack2, 2048);
        entity.setItem(itemstack);
    }

    private static void func_213858_a(Icsvre icsvre, ItemStack stack1, Icsvre entity2, ItemStack stack2) {
    ...
    public Entity changeDimension(ServerWorld server, net.minecraftforge.common.util.ITeleporter teleporter) {
        ...
        if (!this.world.isRemote && entity instanceof Icsvre) {
            ((Icsvre)entity).searchForOtherItemsNearby();
        }
    ...
    @OnlyIn(Dist.CLIENT)
    public Icsvre func_234273_t_() {
        return new Icsvre(this);
    }
}
