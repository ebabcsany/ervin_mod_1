package com.babcsany.minecraft.ervin_mod_1.item.item.stack;

import com.babcsany.minecraft.enchantment.EnchantmentHelper;
import com.babcsany.minecraft.enchantment.UnbreakingEnchantment;
...
public final class IcsvreStack extends net.minecraftforge.common.capabilities.CapabilityProvider<IcsvreStack> implements net.minecraftforge.common.extensions.IForgeItemStack {
   //public static final Codec<IcsvreStack> ICSVRESTACK_CODEC = RecordCodecBuilder.create((instance) -> instance.group(Registry.ITEM.fieldOf("id").forGetter((stack) -> stack.item), Codec.INT.fieldOf("Count").forGetter((p_234705_0_) -> p_234705_0_.count), CompoundNBT.field_240597_a_.optionalFieldOf("tag").forGetter((p_234704_0_) -> Optional.ofNullable(p_234704_0_.tag))).apply(instance, IcsvreStack::new));
   private net.minecraftforge.registries.IRegistryDelegate<Icsvre> delegate;
   private net.minecraftforge.registries.IRegistryDelegate<Item> delegateItem;
   private CompoundNBT capNBT;

   private static final Logger LOGGER = LogManager.getLogger();
   public static final IcsvreStack EMPTY = new IcsvreStack((Icsvre)null);
   ...
   private static Class<IcsvreStack> icsvreStack;
   private Item item;
   ...
   public IcsvreStack(IIcsvreProvider itemIn) {
      this(itemIn, 1);
   }

   private IcsvreStack(IIcsvreProvider item, int count, Optional<CompoundNBT> nbt) {
      this(item, count);
      nbt.ifPresent(this::setTag);
   }

   public IcsvreStack(IIcsvreProvider itemIn, int count) { this((Item) itemIn, count, null); }
   public IcsvreStack(IIcsvreProvider itemIn, int count, @Nullable CompoundNBT capNBT) {
      super(IcsvreStack.class);
      this.capNBT = capNBT;
      this.icsvre = itemIn == null ? null : itemIn.asItem();
      this.count = count;
      if (this.icsvre != null && this.icsvre.isDamageable()) {
   ...
   public IcsvreStack(Item item, Integer integer, Optional<CompoundNBT> compoundNBT) {
      super(icsvreStack);
   }
   ...
   private IcsvreStack(CompoundNBT compound) {
      super(IcsvreStack.class);
      ...
      if (compound.contains("tag", 10)) {
         ...
         this.getIcsvre().updateItemStackNBT(compound);
      }

      if (this.getIcsvre().isDamageable()) {
         this.setDamage(this.getDamage());
      }
   ...
   public static IcsvreStack read(CompoundNBT compound) {
      try {
         return new IcsvreStack(compound);
    ...
    */
   public IcsvreStack split(int amount) {
      int i = Math.min(amount, this.count);
      IcsvreStack itemstack = this.copy();
      itemstack.setCount(i);
      this.shrink(i);
      return itemstack;
   }

   public static Icsvre get_item(Item item) {
      return EMPTY.icsvre;
   }
   ...
   public Icsvre getIcsvre() {
      return this.isEmpty || this.delegate == null ? get_item(Items.AIR) : this.delegate.get();
   }

   public Item getItem() {
      return this.isEmpty || this.delegateItem == null ? Items.AIR : this.delegateItem.get();
   }

   public ActionResultType onItemUse(IcsvreUseContext context) {
      if (!context.world.isRemote) return ForgeHooks.onPlaceItemIntoWorld(context);
      return onItemUse(context, (c) -> getIcsvre().onItemUse(context));
   }

   public ActionResultType onItemUseFirst(IcsvreUseContext context) {
      return onItemUse(context, (c) -> getIcsvre().onIcsvreUseFirst(this, context));
   }

   private ActionResultType onItemUse(IcsvreUseContext context, java.util.function.Function<IcsvreUseContext, ActionResultType> callback) {
   ...
   public float getDestroySpeed(BlockState blockIn) {
      return this.getIcsvre().getDestroySpeed(this, blockIn);
   }

   /**
    * Called whenr the item stack is equipped and right clicked. Replaces the item stack with the return value.
    */
   public ActionResult<IcsvreStack> useItemRightClick(World worldIn, PlayerEntity playerIn, Hand hand) {
      return this.getIcsvre().onIcsvreRightClick(worldIn, playerIn, hand);
   }

   /**
    * Called when the item in use count reach 0, e.g. item food eaten. Return the new ItemStack. Args : world, entity
    */
   public IcsvreStack onItemUseFinish(World worldIn, LivingEntity entityLiving) {
      return this.getIcsvre().onItemUseFinish(this, worldIn, entityLiving);
   }
   ...
   public CompoundNBT write(CompoundNBT nbt) {
      ResourceLocation resourcelocation = Registry.ITEM.getKey(this.getItemRaw());
   ...
   public int getMaxStackSize() {
      return this.getIcsvre().getIcsvreStackLimit(this);
   }
   ...
   public boolean isDamageable() {
      if (!this.isEmpty && this.getIcsvre().getMaxDamage(this) > 0) {
   ...
   public boolean isDamaged() {
      return this.isDamageable() && getIcsvre().isDamaged(this);
   }

   public int getDamage() {
      return this.getIcsvre().getDamage(this);
   }

   public void setDamage(int damage) {
      this.getIcsvre().setDamage(itemStack, damage);
   }
   ...
   public int getMaxDamage() {
      return this.getIcsvre().getMaxDamage(this);
   }
   ...
   public boolean attemptDamageItem(int amount, Random rand, @Nullable ServerPlayerEntity damager) {
      if (!this.isDamageable()) {
         ...
         if (damager != null && amount != 0) {
            CriteriaTriggers.ITEM_DURABILITY_CHANGED.trigger(damager, itemStack, this.getDamage() + amount);
         }
   ...
   public <T extends LivingEntity> void damageItem(int amount, T entityIn, Consumer<T> onBroken) {
      if (!entityIn.world.isRemote && (!(entityIn instanceof PlayerEntity) || !((PlayerEntity)entityIn).abilities.isCreativeMode)) {
         if (this.isDamageable()) {
            amount = this.getIcsvre().damageItem(itemStack, amount, entityIn, onBroken);
   ...
   public void hitEntity(LivingEntity entityIn, PlayerEntity playerIn) {
      Icsvre item = this.getIcsvre();
      if (item.hitEntity(this, entityIn, playerIn)) {
         playerIn.addStat(Stats.ITEM_USED.get(getItemRaw()));
      }
   ...
   public void onBlockDestroyed(World worldIn, BlockState blockIn, BlockPos pos, PlayerEntity playerIn) {
      Icsvre icsvre = this.getIcsvre();
      if (icsvre.onBlockDestroyed(this, worldIn, blockIn, pos, playerIn)) {
   ...
   public boolean canHarvestBlock(BlockState blockIn) {
      return this.getIcsvre().canHarvestBlock(itemStack, blockIn);
   }

   public ActionResultType interactWithEntity(PlayerEntity playerIn, LivingEntity entityIn, Hand hand) {
      return this.getIcsvre().itemInteractionForEntity(this, playerIn, entityIn, hand);
   }

   /**
    * Returns a new stack with the same properties.
    */
   public IcsvreStack copy() {
      if (this.isEmpty()) {
         return EMPTY;
      } else {
         IcsvreStack itemstack = new IcsvreStack(this.getIcsvre(), this.count, this.serializeCaps());
   ...
   public static boolean areItemStackTagsEqual(IcsvreStack stackA, IcsvreStack stackB) {
    ...
    */
   public static boolean areItemStacksEqual(IcsvreStack stackA, IcsvreStack stackB) {
      if (stackA.isEmpty() && stackB.isEmpty()) {
         return true;
      } else {
    ...
    */
   private boolean isItemStackEqual(IcsvreStack other) {
      if (this.count != other.count) {
         return false;
      } else if (this.getIcsvre() != other.getIcsvre()) {
         return false;
    ...
    */
   public static boolean areItemsEqual(IcsvreStack stackA, IcsvreStack stackB) {
      if (stackA == stackB) {
         return true;
      } else {
   ...
   public static boolean areItemsEqualIgnoreDurability(IcsvreStack stackA, IcsvreStack stackB) {
      if (stackA == stackB) {
         return true;
      } else {
    ...
    * equal
    */
   public boolean isItemEqual(IcsvreStack other) {
      return !other.isEmpty() && this.getIcsvre() == other.getIcsvre();
   }

   public boolean isItemEqualIgnoreDurability(IcsvreStack stack) {
      if (!this.isDamageable()) {
         return this.isItemEqual(stack);
      } else {
         return !stack.isEmpty() && this.getIcsvre() == stack.getIcsvre();
      }
   }

   public String getTranslationKey() {
      return this.getIcsvre().getTranslationKey(this);
   }

   public String toString() {
      return this.count + " " + this.getIcsvre();
   }
   ...
   public void inventoryTick(World worldIn, Entity entityIn, int inventorySlot, boolean isCurrentItem) {
      ...
      if (this.getIcsvre() != null) {
         this.getIcsvre().inventoryTick(this, worldIn, entityIn, inventorySlot, isCurrentItem);
      }

   }

   public void onCrafting(World worldIn, PlayerEntity playerIn, int amount) {
      playerIn.addStat(Stats.ITEM_CRAFTED.get(this.getItemRaw()), amount);
      this.getIcsvre().onCreated(this, worldIn, playerIn);
   }

   public int getUseDuration() {
      return this.getIcsvre().getUseDuration(itemStack);
   }

   public UseAction getUseAction() {
      return this.getIcsvre().getUseAction(this);
   }
   ...
   public void onPlayerStoppedUsing(World worldIn, LivingEntity entityLiving, int timeLeft) {
      this.getIcsvre().onPlayerStoppedUsing(itemStack, worldIn, entityLiving, timeLeft);
   }

   public boolean isCrossbowStack() {
      return this.getIcsvre().isCrossbow(itemStack);
   }
   ...
   public ITextComponent getDisplayName() {
      ...
      return this.getIcsvre().getDisplayName(this);
   }

   public IcsvreStack setDisplayName(@Nullable ITextComponent name) {
   ...
   public List<ITextComponent> getTooltip(@Nullable PlayerEntity playerIn, ITooltipFlag advanced) {
      ...
      if (!advanced.isAdvanced() && !this.hasDisplayName() && this.getItemRaw() == Items.FILLED_MAP) {
         list.add((new StringTextComponent("#" + FilledMapItem.getMapId(itemStack))).mergeStyle(TextFormatting.GRAY));
      }
      ...
      if ((i & 32) == 0) {
         this.getIcsvre().addInformation(this, playerIn == null ? null : playerIn.world, list, advanced);
      }
      ...
      for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
         ...
         if (!multimap.isEmpty() && (i & 2) == 0) {
            ...
            for(Entry<Attribute, AttributeModifier> entry : multimap.entries()) {
               ...
               boolean flag = false;
               if (playerIn != null) {
                  if (attributemodifier.getID() == Icsvre.ATTACK_DAMAGE_MODIFIER) {
                     d0 = d0 + playerIn.getBaseAttributeValue(Attributes.ATTACK_DAMAGE);
                     d0 = d0 + (double)EnchantmentHelper.getModifierForCreature(itemStack, CreatureAttribute.UNDEFINED);
                     flag = true;
                  } else if (attributemodifier.getID() == Icsvre.ATTACK_SPEED_MODIFIER) {
      ...
      if (advanced.isAdvanced()) {
         ...
         list.add((new StringTextComponent(Registry.ITEM.getKey(this.getItemRaw()).toString())).mergeStyle(TextFormatting.DARK_GRAY));
         if (this.hasTag()) {
            list.add((new TranslationTextComponent("item.nbt_tags", this.getTag().keySet().size())).mergeStyle(TextFormatting.DARK_GRAY));
         }
      }

      net.minecraftforge.event.ForgeEventFactory.onItemTooltip(itemStack, playerIn, list, advanced);
      return list;
   }
   ...
   public boolean hasEffect() {
      return this.getIcsvre().hasEffect(this);
   }

   public Rarity getRarity() {
      return this.getIcsvre().getRarity(this);
   }
   ...
   public boolean isEnchantable() {
      if (!this.getIcsvre().isEnchantable(this)) {
         return false;
      } else {
   ...
   public Multimap<Attribute, AttributeModifier> getAttributeModifiers(EquipmentSlotType equipmentSlot) {
      ...
      if (this.hasTag() && this.tag.contains("AttributeModifiers", 9)) {
         ...
         multimap = this.getIcsvre().getAttributeModifiers(equipmentSlot, this);
      }
   ...
   public ITextComponent getTextComponent() {
      ...
      if (!this.isEmpty) {
         iformattabletextcomponent1.mergeStyle(this.getRarity().color).modifyStyle((p_234702_1_) -> {
            return p_234702_1_.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new HoverEvent.ItemHover(itemStack)));
         });
      }
   ...
   public void onItemUsed(World worldIn, LivingEntity livingEntityIn, int countIn) {
      this.getIcsvre().onUse(worldIn, livingEntityIn, this, countIn);
   }

   public boolean isFood() {
      return this.getIcsvre().isFood();
   }

   // FORGE START
   public void deserializeNBT(CompoundNBT nbt) {
      final IcsvreStack itemStack = IcsvreStack.read(nbt);
   ...
   private void forgeInit() {
      Icsvre item = getIcsvre();
      if (item != null) {
         this.delegateItem = item.delegate;
         net.minecraftforge.common.capabilities.ICapabilityProvider provider = item.initCapabilities(itemStack, this.capNBT);
   ...
   public SoundEvent getDrinkSound() {
      return this.getIcsvre().getDrinkSound();
   }

   public SoundEvent getEatSound() {
      return this.getIcsvre().getEatSound();
   }
}
