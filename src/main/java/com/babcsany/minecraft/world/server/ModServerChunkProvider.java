...
public class ModServerChunkProvider extends AbstractChunkProvider {
   private static final List<ChunkStatus> field_217239_c = ChunkStatus.getAll();
   private S ticketManager;
   private S.L chunk_manager;
   private S.B chunk_1_manager;
   public final ChunkGenerator generator;
   public final ModServerWorld world;
   ...
   public ModServerChunkProvider(ModServerWorld modServerWorld, SaveFormat.LevelSave levelSave, DataFixer dataFixer, TemplateManager templateManager, Executor executor, ChunkGenerator chunkGenerator, int p_i232603_7_, boolean p_i232603_8_, IChunkStatusListener iChunkStatusListener, Supplier<DimensionSavedDataManager> dimensionSavedDataManagerSupplier) {
      this.world = modServerWorld;
      this.executor = new ChunkExecutor(modServerWorld.getWorld());
      this.generator = chunkGenerator;
      this.mainThread = Thread.currentThread();
      File file1 = levelSave.func_237291_a_(modServerWorld.world.func_234923_W_());
      File file2 = new File(file1, "data");
      file2.mkdirs();
      this.savedData = new DimensionSavedDataManager(file2, dataFixer);
      this.chunkManager = new ChunkManager(modServerWorld.getWorldServer(), levelSave, dataFixer, templateManager, executor, this.executor, this, this.getChunkGenerator(), iChunkStatusListener, dimensionSavedDataManagerSupplier, p_i232603_7_, p_i232603_8_);
      this.lightManager = this.chunk_manager.get_LightManager();
   ...
   private ChunkHolder func_217213_a(long chunkPosIn) {
      return this.chunk_manager.func_219219_b_(chunkPosIn);
   }
   ...
   public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_217232_b(int chunkX, int chunkZ, ChunkStatus requiredStatus, boolean load) {
      ...
      if (flag) {
         ...
         completablefuture = CompletableFuture.supplyAsync(() -> this.func_217233_c(chunkX, chunkZ, requiredStatus, load), this.executor).thenCompose((p_217211_0_) -> {
            return p_217211_0_;
         });
      }
   ...
   private boolean func_217224_a(@Nullable ChunkHolder modChunkHolderIn, int p_217224_2_) {
      return modChunkHolderIn == null || modChunkHolderIn.getChunkLevel() > p_217224_2_;
   }
   ...
   private boolean func_217235_l() {
      ...
      boolean flag1 = this.chunk_manager.refresh_Off_ThreadCache();
      if (!flag && !flag1) {
         return false;
      } else {
         this.invalidateCaches();
   ...
   public void save(boolean flush) {
      this.func_217235_l();
      this.chunk_manager.save_1(flush);
   }
   ...
   private void tickChunks() {
      ...
      if (!flag) {
         ...
         this.world.getProfiler().endSection();
         List<ChunkHolder> list = Lists.newArrayList(this.chunk_manager.getLoadedChunksIterable());
         ...
         list.forEach((p_241099_7_) -> {
            ...
            if (optional.isPresent()) {
               ...
               if (optional1.isPresent()) {
                  ...
                  if (!this.chunk_manager.isOutsideSpawningRadius(chunkpos)) {
                     chunk.setInhabitedTime(chunk.getInhabitedTime() + j);
                     if (flag1 && (this.spawnHostiles || this.spawnPassives) && this.world.getWorldBorder().contains(chunk.getPos())) {
                        WorldEntitySpawner.func_234979_a_(this.world.getWorldServer(), chunk, worldentityspawner$entitydensitymanager, this.spawnPassives, this.spawnHostiles, flag2);
                     }
      ...
      this.chunk_manager.tickEntityTracker();
   }
   ...
   public void untrack(Entity entityIn) {
      this.chunk_manager.untrack(entityIn);
   }

   public void track(Entity entityIn) {
      this.chunk_manager.track(entityIn);
   }

   public void sendToTrackingAndSelf(Entity entityIn, IPacket<?> packet) {
      this.chunk_manager.sendToTrackingAndSelf(entityIn, packet);
   }

   public void sendToAllTracking(Entity entityIn, IPacket<?> packet) {
      this.chunk_manager.sendToAllTracking(entityIn, packet);
   }

   public void setViewDistance(int viewDistance) {
      this.chunk_manager.setViewDistance(viewDistance);
   }
   ...
   public PointOfInterestManager getPointOfInterestManager() {
      return this.chunk_manager.getPointOfInterestManager();
   }
   ...
   final class ChunkExecutor extends ThreadTaskExecutor<Runnable> {
      ...
      protected Thread getExecutionThread() {
         return ModServerChunkProvider.this.mainThread;
      }

      protected void run(Runnable taskIn) {
         ModServerChunkProvider.this.world.getProfiler().func_230035_c_("runTask");
         super.run(taskIn);
      }

      protected boolean driveOne() {
         if (ModServerChunkProvider.this.func_217235_l()) {
            return true;
         } else {
            ModServerChunkProvider.this.lightManager.func_215588_z_();
