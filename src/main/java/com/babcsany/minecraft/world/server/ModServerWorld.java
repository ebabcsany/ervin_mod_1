...
public class ModServerWorld extends world implements ISeedReader, IForgeWorldServer, DataFixer {
   ...
   private final ServerTickList<Block> pendingBlockTicks = new ServerTickList<>(serverWorld, (T) -> T == null || T.getDefaultState().isAir(), Registry.BLOCK::getKey, this::tickBlock);
   private final ServerTickList<Fluid> pendingFluidTicks = new ServerTickList<>(serverWorld, (p_205774_0_) -> p_205774_0_ == null || p_205774_0_ == Fluids.EMPTY, Registry.FLUID::getKey, this::tickFluid);
   ...
   private final List<ISpecialSpawner> iSpecialSpawnerList;
   @Nullable
   ...
   public ModServerWorld(MinecraftServer minecraftServer, Executor executor, SaveFormat.LevelSave p_i232604_3_, IServerWorldInfo serverWorldInfo, RegistryKey<world> worldRegistryKey, RegistryKey<dimensionType> dimensionTypeRegistryKey, dimensionType dimensionType, IChunkStatusListener chunkStatusListener, ChunkGenerator chunkGenerator, boolean b, long l, List<ISpecialSpawner> iSpecialSpawnerList, boolean b1) {
      super(serverWorldInfo, worldRegistryKey, dimensionTypeRegistryKey, dimensionType, minecraftServer::getProfiler, false, b, l);
      this.aBoolean = b1;
      ...
      this.raids = this.getSavedData().getOrCreate(() -> new RaidManager(serverWorld), RaidManager.func_234620_a_(this.func_230315_m_()));
      if (!minecraftServer.isSinglePlayer()) {
         serverWorldInfo.setGameType(minecraftServer.getGameType());
      }
      ...
      if (this.func_230315_m_().func_236046_h_()) {
         this.field_241105_O_ = new DragonFightManager(serverWorld, minecraftServer.func_240793_aU_().getDimensionGeneratorSettings().getSeed(), minecraftServer.func_240793_aU_().getDragonFightData());
      } else {
   ...
   public void func_241113_a_(int p_241113_1_, int p_241113_2_, boolean p_241113_3_, boolean p_241113_4_) {
      this.iServerWorldInfo.func_230391_a_(p_241113_1_);
      this.iServerWorldInfo.setRainTime(p_241113_2_);
      this.iServerWorldInfo.setThunderTime(p_241113_2_);
      this.iServerWorldInfo.setRaining(p_241113_3_);
      this.iServerWorldInfo.setThundering(p_241113_4_);
   }
   ...
   public void tick(BooleanSupplier hasTimeLeft) {
      ...
      if (this.func_230315_m_().hasSkyLight()) {
         if (this.getGameRules().getBoolean(GameRules.DO_WEATHER_CYCLE)) {
            int i = this.iServerWorldInfo.func_230395_g_();
            int j = this.iServerWorldInfo.getThunderTime();
            int k = this.iServerWorldInfo.getRainTime();
            ...
            this.iServerWorldInfo.setThunderTime(j);
            this.iServerWorldInfo.setRainTime(k);
            this.iServerWorldInfo.func_230391_a_(i);
            this.iServerWorldInfo.setThundering(flag1);
            this.iServerWorldInfo.setRaining(flag2);
         }
      ...
      if (this.prevRainingStrength != this.rainingStrength) {
         this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.field_241771_h_, this.rainingStrength), serverWorld.func_234923_W_());
      }

      if (this.prevThunderingStrength != this.thunderingStrength) {
         this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.field_241772_i_, this.thunderingStrength), serverWorld.func_234923_W_());
      }
      ...
      if (flag != this.isRaining()) {
         if (flag) {
            this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.field_241766_c_, 0.0F), serverWorld.func_234923_W_());
         } else {
            this.server.getPlayerList().func_232642_a_(new SChangeGameStatePacket(SChangeGameStatePacket.field_241765_b_, 0.0F), serverWorld.func_234923_W_());
         }
      ...
      if (this.allPlayersSleeping && this.players.stream().noneMatch((p_241132_0_) -> {
         ...
         if (this.getGameRules().getBoolean(GameRules.DO_DAYLIGHT_CYCLE)) {
            ...
            this.func_241114_a_(net.minecraftforge.event.ForgeEventFactory.onSleepFinished(serverWorld, l - l % 24000L, this.getDayTime()));
         }
   ...
   protected void func_241126_b_() {
      if (this.aBoolean) {
         ...
         this.iServerWorldInfo.setGameTime(i);
         this.iServerWorldInfo.getScheduledEvents().run(this.server, i);
   ...
   public void func_241114_a_(long p_241114_1_) {
      this.iServerWorldInfo.setDayTime(p_241114_1_);
   }

   public void func_241123_a_(boolean p_241123_1_, boolean p_241123_2_) {
      for(ISpecialSpawner ispecialspawner : this.iSpecialSpawnerList) {
         ispecialspawner.func_230253_a_(serverWorld, p_241123_1_, p_241123_2_);
      }
   ...
   public void tickEnvironment(Chunk chunkIn, int randomTickSpeed) {
      ...
      if (flag && this.isThundering() && this.rand.nextInt(100000) == 0) {
         ...
         if (this.isRainingAt(blockpos)) {
            ...
            if (flag1) {
               SkeletonHorseEntity skeletonhorseentity = EntityType.SKELETON_HORSE.create(serverWorld);
      ...
      if (this.rand.nextInt(16) == 0) {
         ...
         if (flag && this.getBiome(blockpos3).getPrecipitation() == Biome.RainType.RAIN) {
            this.getBlockState(blockpos3).getBlock().fillWithRain(serverWorld, blockpos3);
         }
      }
            ...
            if (chunksection != Chunk.EMPTY_SECTION && chunksection.needsRandomTickAny()) {
               ...
               for(int l = 0; l < randomTickSpeed; ++l) {
                  ...
                  if (blockstate.ticksRandomly()) {
                     blockstate.randomTick(serverWorld, blockpos1, this.rand);
                  }

                  FluidState fluidstate = blockstate.getFluidState();
                  if (fluidstate.ticksRandomly()) {
                     fluidstate.randomTick(serverWorld, blockpos1, this.rand);
                  }
   ...
   private void resetRainAndThunder() {
      this.iServerWorldInfo.setRainTime(0);
      this.iServerWorldInfo.setRaining(false);
      this.iServerWorldInfo.setThunderTime(0);
      this.iServerWorldInfo.setThundering(false);
   }
   ...
   private void tickFluid(NextTickListEntry<Fluid> fluidTickEntry) {
      ...
      if (fluidstate.getFluid() == fluidTickEntry.getTarget()) {
         fluidstate.tick(serverWorld, fluidTickEntry.position);
      }
   ...
   private void tickBlock(NextTickListEntry<Block> blockTickEntry) {
      ...
      if (blockstate.isIn(blockTickEntry.getTarget())) {
         blockstate.tick(serverWorld, blockTickEntry.position, this.rand);
      }
   ...
   public boolean isBlockModifiable(PlayerEntity player, BlockPos pos) {
      return !this.server.isBlockProtected(serverWorld, pos, player) && this.getWorldBorder().contains(pos);
   }
   ...
   private void addPlayer(ServerPlayerEntity player) {
      if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(player, serverWorld))) return;
   ...
   private boolean addEntity0(Entity entityIn) {
      if (entityIn.removed) {
         ...
         if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entityIn, serverWorld))) return false;
   ...
   public void removeEntityComplete(Entity entityIn, boolean keepData) {
      ...
      net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.entity.EntityLeaveWorldEvent(entityIn, serverWorld));
   }
   ...
   public void sendBlockBreakProgress(int breakerId, BlockPos pos, int progress) {
      for(ServerPlayerEntity serverplayerentity : this.server.getPlayerList().getPlayers()) {
         if (serverplayerentity != null && serverplayerentity.world == serverWorld && serverplayerentity.getEntityId() != breakerId) {
   ...
   public void playSound(@Nullable PlayerEntity player, double x, double y, double z, SoundEvent soundIn, SoundCategory category, float volume, float pitch) {
      ...
      this.server.getPlayerList().sendToAllNearExcept(player, x, y, z, volume > 1.0F ? (double)(16.0F * volume) : 16.0D, serverWorld.func_234923_W_(), new SPlaySoundEffectPacket(soundIn, category, x, y, z, volume, pitch));
   }
   ...
   public void playMovingSound(@Nullable PlayerEntity playerIn, Entity entityIn, SoundEvent eventIn, SoundCategory categoryIn, float volume, float pitch) {
      ...
      volume = event.getVolume();
      this.server.getPlayerList().sendToAllNearExcept(playerIn, entityIn.getPosX(), entityIn.getPosY(), entityIn.getPosZ(), volume > 1.0F ? (double)(16.0F * volume) : 16.0D, serverWorld.func_234923_W_(), new SSpawnMovingSoundEffectPacket(eventIn, categoryIn, entityIn, volume, pitch));
   }
   ...
   public void playEvent(@Nullable PlayerEntity player, int type, BlockPos pos, int data) {
      this.server.getPlayerList().sendToAllNearExcept(player, pos.getX(), pos.getY(), pos.getZ(), 64.0D, serverWorld.func_234923_W_(), new SPlaySoundEventPacket(type, pos, data, false));
   }
   ...
   public Explosion createExplosion(@Nullable Entity exploder, @Nullable DamageSource damageSource, @Nullable IExplosionContext context, double x, double y, double z, float size, boolean causesFire, Explosion.Mode mode) {
      Explosion explosion = new Explosion(serverWorld, exploder, damageSource, context, x, y, z, size, causesFire, mode);
      if (net.minecraftforge.event.ForgeEventFactory.onExplosionStart(serverWorld, explosion)) return explosion;
   ...
   private void sendQueuedBlockEvents() {
      while(!this.blockEventQueue.isEmpty()) {
         ...
         if (this.fireBlockEvent(blockeventdata)) {
            this.server.getPlayerList().sendToAllNearExcept(null, (double)blockeventdata.getPosition().getX(), (double)blockeventdata.getPosition().getY(), (double)blockeventdata.getPosition().getZ(), 64.0D, serverWorld.func_234923_W_(), new SBlockActionPacket(blockeventdata.getPosition(), blockeventdata.getBlock(), blockeventdata.getEventID(), blockeventdata.getEventParameter()));
         }
      }
   ...
   private boolean fireBlockEvent(BlockEventData event) {
      ...
      return blockstate.isIn(event.getBlock()) ? blockstate.receiveBlockEvent(serverWorld, event.getPosition(), event.getEventID(), event.getEventParameter()) : false;
   }
   ...
   private boolean sendPacketWithinDistance(ServerPlayerEntity player, boolean longDistance, double posX, double posY, double posZ, IPacket<?> packet) {
      if (player.getServerWorld() != serverWorld) {
         return false;
      } else {
   ...
   public BlockPos func_241117_a_(Structure<?> p_241117_1_, BlockPos p_241117_2_, int p_241117_3_, boolean p_241117_4_) {
      return !this.server.func_240793_aU_().getDimensionGeneratorSettings().doesGenerateFeatures() ? null : this.getChunkProvider().getChunkGenerator().func_235956_a_(serverWorld, p_241117_1_, p_241117_2_, p_241117_3_, p_241117_4_);
   }
      ...
      if (!Objects.equals(optional, optional1)) {
         ...
         optional.ifPresent((p_241130_2_) -> {
            this.getServer().execute(() -> {
               ...
               DebugPacketSender.func_218805_b(serverWorld, blockpos);
            });
         });
         optional1.ifPresent((p_217476_2_) -> {
            this.getServer().execute(() -> {
               this.getPointOfInterestManager().add(blockpos, p_217476_2_);
               DebugPacketSender.func_218799_a(serverWorld, blockpos);
            });
         });
      }
   }
   ...
   public void writeDebugInfo(Path pathIn) throws IOException {
      ...
      try (Writer writer = Files.newBufferedWriter(pathIn.resolve("stats.txt"))) {
         ...
         writer.write("distance_manager: " + ticketManager.func_225412_c() + "\n");
      ...
      try (Writer writer2 = Files.newBufferedWriter(path)) {
         modChunkManager.func_225406_a(writer2);
      }
   ...
   public String toString() {
      return "ServerLevel[" + this.iServerWorldInfo.getWorldName() + "]";
   }
   ...
   public static void func_241121_a_(ModServerWorld p_241121_0_) {
