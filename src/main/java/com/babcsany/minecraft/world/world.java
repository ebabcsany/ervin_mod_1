...
public abstract class world extends net.minecraftforge.common.capabilities.CapabilityProvider<world> implements IWorld, I_world, AutoCloseable, net.minecraftforge.common.extensions.IForgeWorld {
    ...
    public static boolean isYOutOfBounds(int y) {
        return y < isYOutOfBoundsDown || y >= isYOutOfBoundsUp;
    }

    public double func_234936_m_(BlockPos p_234936_1_) {
        return this.func_234932_c_(p_234936_1_, (p_234927_0_) -> false);
    }
        ...
        {
            {
                if (blockstate1 == state) {
                    ...
                    if ((flags & 2) != 0 && (!this.isRemote || (flags & 4) == 0) && (this.isRemote || chunk.getLocationType() != null && chunk.getLocationType().isAtLeast(ChunkHolder.LocationType.TICKING))) {
                        world.notifyBlockUpdate(pos, blockstate, state, flags);
                    }
    ...
    public void notifyNeighborsOfStateChange(BlockPos pos, Block blockIn) {
        if (net.minecraftforge.event.ForgeEventFactory.onNeighborNotify(world, pos, this.getBlockState(pos), java.util.EnumSet.allOf(Direction.class), false).isCanceled())
            return;
    ...
    public void notifyNeighborsOfStateExcept(BlockPos pos, Block blockType, Direction skipSide) {
        ...
        if (net.minecraftforge.event.ForgeEventFactory.onNeighborNotify(world, pos, this.getBlockState(pos), directions, false).isCanceled())
            return;
    ...
    public void neighborChanged(BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!this.isRemote) {
            ...
            try {
                blockstate.neighborChanged(world, pos, blockIn, fromPos, false);
    ...
    public int getHeight(net.minecraft.world.gen.Heightmap.Type heightmapType, int x, int z) {
        int i;
    ...
    public void playSound(@Nullable PlayerEntity player, BlockPos pos, SoundEvent soundIn, SoundCategory category, float volume, float pitch) {
        world.playSound(player, (double)pos.getX() + 0.5D, (double)pos.getY() + 0.5D, (double)pos.getZ() + 0.5D, soundIn, category, volume, pitch);
    }
    ...
    public boolean addTileEntity(TileEntity tile) {
        if (tile.getWorld() != world) tile.setWorldAndPos(world, tile.getPos()); // Forge - set the world early as vanilla doesn't set it until next tick
        if (this.processingLoadedTiles) {
            LOGGER.error("Adding block entity while ticking: {} @ {}", () -> Registry.BLOCK_ENTITY_TYPE.getKey(tile.getType()), tile::getPos);
    ...
    public void addTileEntities(Collection<TileEntity> tileEntityCollection) {
        if (this.processingLoadedTiles) {
            tileEntityCollection.stream().filter(tileEntity -> tileEntity.getWorld() != world).forEach(te -> te.setWorldAndPos(world, te.getPos())); // Forge - set the world early as vanilla doesn't set it until next tick
            ...
            for(int i = 0; i < this.addedTileEntityList.size(); ++i) {
                ...
                if (!tileentity1.isRemoved()) {
                    ...
                    if (this.isBlockLoaded(tileentity1.getPos())) {
                        ...
                        world.notifyBlockUpdate(tileentity1.getPos(), blockstate, blockstate, 3);
                    }
                }
            }
    ...
    public Explosion createExplosion(@Nullable Entity exploder, @Nullable DamageSource damageSource, @Nullable IExplosionContext context, double x, double y, double z, float size, boolean causesFire, Explosion.Mode mode) {
        Explosion explosion = new Explosion(world, exploder, damageSource, context, x, y, z, size, causesFire, mode);
    ...
    public void setTileEntity(BlockPos pos, @Nullable TileEntity tileEntityIn) {
        if (!isOutsideBuildHeight(pos)) {
            ...
            if (tileEntityIn != null && !tileEntityIn.isRemoved()) {
                if (this.processingLoadedTiles) {
                    tileEntityIn.setWorldAndPos(world, pos);
    ...
    public com.babcsany.minecraft.world.world getModWorld() {
        return this;
    }
    ...
    public void addBlockEvent(BlockPos pos, Block blockIn, int eventID, int eventParam) {
        this.getBlockState(pos).receiveBlockEvent(world, pos, eventID, eventParam);
    }
            ...
            if (this.isBlockLoaded(blockpos)) {
                ...
                if (blockstate.isNormalCube(this, blockpos)) {
                    ...
                    if (blockstate.getWeakChanges(this, blockpos)) {
                        blockstate.neighborChanged(world, blockpos, blockIn, pos, false);
                    }
                }
            }
        }
