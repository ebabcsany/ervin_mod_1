...
@OnlyIn(Dist.CLIENT)
public class WorldRenderer extends net.minecraft.client.renderer.WorldRenderer implements IResourceManagerReloadListener, AutoCloseable {
   ...
   private static final ResourceLocation ASLOFZ_TEXTURES = new ResourceLocation("ervin_mod_1:textures/environment/aslofz.png");
   public static final Direction[] FACINGS = Direction.values();
   private final Minecraft mc;
   private static net.minecraft.client.renderer.WorldRenderer worldRenderer;
   ...
   private ViewFrustum_ viewFrustum;
   ...
   private void renderWeather(LightTexture lightmapIn, float partialTicks, double xIn, double yIn, double zIn) {
                     ...
                     if (f2 >= 0.15F) {
                        ...
                        if (f2 >= 120.0F) {
                           if (i1 != 0) {
                              ...
                              i1 = 0;
                              this.mc.getTextureManager().bindTexture(ASLOFZ_TEXTURES);
                              bufferbuilder.begin(7, DefaultVertexFormats.PARTICLE_POSITION_TEX_COLOR_LMAP);
                           }
                        }
   ...
   public void loadRenderers() {
      if (this.world != null) {
         ...
         if (this.renderDispatcher == null) {
            this.renderDispatcher = new ChunkRenderDispatcher(this.world, worldRenderer, Util.getServerExecutor(), this.mc.isJava64bit(), this.renderTypeTextures.getFixedBuilder());
         } else {
         ...
         this.viewFrustum = new ViewFrustum_(this.renderDispatcher, this.world, this.mc.gameSettings.renderDistanceChunks, worldRenderer);
         if (this.world != null) {
   ...
   private void setupTerrain(ActiveRenderInfo activeRenderInfoIn, ClippingHelper camera, boolean debugCamera, int frameCount, boolean playerSpectator) {
      ...
      if (!debugCamera && this.displayListEntitiesDirty) {
         ...
         if (chunkrenderdispatcher$chunkrender != null) {
            ...
            queue.add(new LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender, (Direction)null, 0));
         } else {
   ...
   public void updateCameraAndRender(MatrixStack matrixStackIn, float partialTicks, long finishTimeNano, boolean drawBlockOutline, ActiveRenderInfo activeRenderInfoIn, GameRenderer gameRendererIn, LightTexture lightmapIn, Matrix4f projectionIn) {
      ...
      if (drawBlockOutline && raytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
         ...
         if (!net.minecraftforge.client.ForgeHooksClient.onDrawBlockHighlight(worldRenderer, activeRenderInfoIn, mc.objectMouseOver, partialTicks, matrixStackIn, irendertypebuffer$impl))
      ...
      if (this.field_239227_K_ != null) {
         ...
         RenderState_.targetState.setupRenderState();
         ...
         RenderState_.targetState.clearRenderState();
      } else {
         iprofiler.endStartSection("translucent");
         this.renderBlockLayer(RenderType.getTranslucent(), matrixStackIn, d0, d1, d2);
         iprofiler.endStartSection("string");
      ...
      if (this.field_239227_K_ != null) {
         RenderState_.targetState1.setupRenderState();
         iprofiler.endStartSection("weather");
         this.renderWeather(lightmapIn, partialTicks, d0, d1, d2);
         this.renderWorldBorder(activeRenderInfoIn);
         RenderState_.targetState1.clearRenderState();
