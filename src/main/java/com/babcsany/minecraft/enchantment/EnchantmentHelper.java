...
public class EnchantmentHelper {
   ...
   /**
    * Returns the level of enchantment on the IcsvreStack passed.
    */
   public static int getEnchantmentLevel(Enchantment enchID, IcsvreStack stack) {
      if (stack.isEmpty()) {
         return 0;
      } else {
    ...
    */
   public static Map<Enchantment, Integer> getEnchantments(IcsvreStack stack) {
      ListNBT listnbt = stack.getIcsvre().getItem() == Items.ENCHANTED_BOOK ? com.babcsany.minecraft.item.EnchantedBookItem.getEnchantments(stack) : stack.getEnchantmentTagList();
   ...
   private static EquipmentSlotType[] applicableEquipmentTypes;
   public static Map<EquipmentSlotType, IcsvreStack> getEntityEquipment(LivingEntity livingEntityIn) {
      Map<EquipmentSlotType, IcsvreStack> map = Maps.newEnumMap(EquipmentSlotType.class);

      for(EquipmentSlotType equipmentslottype : applicableEquipmentTypes) {
         IcsvreStack itemstack = living.getIcsvreStackFromSlot(equipmentslottype);
   ...
   public static int getMaxEnchantmentLevel(Enchantment enchantmentIn, LivingEntity entityIn) {
      Iterable<IcsvreStack> iterable = getEntityEquipment(entityIn).values();
      if (iterable == null) {
         return 0;
      } else {
         int i = 0;

         for(IcsvreStack itemstack : iterable) {
   ...
   public static int getFishingLuckBonus(IcsvreStack stack) {
      return getEnchantmentLevel(Enchantments.LUCK_OF_THE_SEA, stack);
   }

   public static int getFishingSpeedBonus(IcsvreStack stack) {
   ...
   public static boolean hasBindingCurse(IcsvreStack stack) {
      return getEnchantmentLevel(Enchantments.BINDING_CURSE, stack) > 0;
   }

   public static boolean hasVanishingCurse(IcsvreStack stack) {
      return getEnchantmentLevel(Enchantments.VANISHING_CURSE, stack) > 0;
   }

   public static int getLoyaltyModifier(IcsvreStack stack) {
      return getEnchantmentLevel(Enchantments.LOYALTY, stack);
   }

   public static int getRiptideModifier(IcsvreStack stack) {
      return getEnchantmentLevel(Enchantments.RIPTIDE, stack);
   }

   public static boolean hasChanneling(IcsvreStack stack) {
    ...
    */
   @Nullable
   public static Entry<EquipmentSlotType, IcsvreStack> getRandomItemWithEnchantment(Enchantment targetEnchantment, LivingEntity entityIn) {
      return func_234844_a_(targetEnchantment, entityIn, (stack) -> true);
   }

   @Nullable
   public static Entry<EquipmentSlotType, IcsvreStack> func_234844_a_(Enchantment enchantment, LivingEntity livingEntity, Predicate<IcsvreStack> stackCondition) {
      Map<EquipmentSlotType, IcsvreStack> map = getEntityEquipment(livingEntity);
      if (map.isEmpty()) {
         return null;
      } else {
         List<Entry<EquipmentSlotType, IcsvreStack>> list = Lists.newArrayList();

         for(Entry<EquipmentSlotType, IcsvreStack> entry : map.entrySet()) {
            IcsvreStack icsvreStack = entry.getValue();
            if (!icsvreStack.isEmpty() && getEnchantmentLevel(enchantment, icsvreStack) > 0 && stackCondition.test(icsvreStack)) {
               list.add(entry);
            }
         }
   ...
   /**
    * Returns the enchantability of icsvreStack, using a separate calculation for each enchantNum (0, 1 or 2), cutting to
    * the max enchantability power of the table, which is locked to a max of 15.
    */
   public static int calcItemStackEnchantability(Random rand, int enchantNum, int power, IcsvreStack stack) {
