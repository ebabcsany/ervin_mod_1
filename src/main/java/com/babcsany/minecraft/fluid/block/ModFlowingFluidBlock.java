...
public class ModFlowingFluidBlock extends net.minecraft.block.FlowingFluidBlock implements IBucketPickupHandler {
   public static final IntegerProperty LEVEL = BlockStateProperties.LEVEL_0_15;
   private final FlowingFluid fluid;
   public static net.minecraft.block.FlowingFluidBlock flowingFluidBlock;
   private final List<FluidState> field_212565_c;
   public static final VoxelShape field_235510_c_ = FlowingFluidBlock.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 8.0D, 16.0D);

   public ModFlowingFluidBlock(FlowingFluid fluidIn, AbstractBlock.Properties builder) {
   ...
   /**
    * @param supplier A fluid supplier such as {@link net.minecraftforge.fml.RegistryObject<net.minecraft.fluid.Fluid>}
    */
   public ModFlowingFluidBlock(java.util.function.Supplier<? extends FlowingFluid> supplier, Properties properties) {
   ...
   public void neighborChanged(BlockState state, World worldIn, BlockPos pos, FlowingFluidBlock blockIn, BlockPos fromPos, boolean isMoving) {
      if (this.reactWithNeighbors(worldIn, pos, state)) {
         worldIn.getPendingFluidTicks().scheduleTick(pos, state.getFluidState().getFluid(), this.fluid.getTickRate(worldIn));
      }
         ...
         for(Direction direction : Direction.values()) {
            if (direction != Direction.DOWN) {
               ...
               if (worldIn.getFluidState(blockpos).isTagged(net.minecraft.tags.FluidTags.WATER)) {
                  FlowingFluidBlock block = (FlowingFluidBlock) (worldIn.getFluidState(pos).isSource() ? BlockItemInit.DURTGURBF.get() : BlockItemInit.DURT.get());
      ...
      if (this.fluid.isIn(net.minecraft.tags.FluidTags.LAVA)) {
         ...
         for(Direction direction : Direction.values()) {
            if (direction != Direction.DOWN) {
               ...
               if (worldIn.getFluidState(blockpos).isTagged(net.minecraft.tags.FluidTags.WATER)) {
                  FlowingFluidBlock block = (FlowingFluidBlock) (worldIn.getFluidState(pos).isSource() ? Blocks.OBSIDIAN : Blocks.COBBLESTONE);
                  worldIn.setBlockState(pos, net.minecraftforge.event.ForgeEventFactory.fireFluidPlaceBlockEvent(worldIn, pos, pos, block.getDefaultState()));
                  this.triggerMixEffects(worldIn, pos);
                  return false;
               }
